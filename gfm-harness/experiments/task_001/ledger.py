"""
Initial ledger for task_001.

This populates the harness with the user + LLM + toolchain collective
derived from the design discussion. The individual capabilities are
...
The cooperative capabilities derive mechanically from pairings of
...
# Adds gfm-harness/ to sys.path if the harness package isn't already importable.
import sys
...
if __name__ == "__main__":
    ...
    if errors:
        ...
        print(f"Agents: {len(ledger.agents)}")
        print(f"Individual capabilities: {ledger.individual_capability_count()}")
        print(f"Cooperative capabilities: {len(ledger.cooperative)}")
        print(f"  cross-substrate: {ledger.cross_substrate_coop_count()}")
        print(f"  same-substrate: {ledger.same_substrate_coop_count()}")
        print(f"Substrate partition: {ledger.substrate_partition()}")
        print(f"m (substrate count): {ledger.m()}")
