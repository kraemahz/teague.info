...
## 1. Critical incidents — importance 1.0, never evict
...
are not repeated. Even after a critical incident has been distilled
into a Lesson capturing the avoidance pattern, the original incident
Entry remains at importance 1.0 as historical record.

  Recognition cues: "lost", "wasted", "destroyed", "had to redo",
                    "ran out of", "failed after N hours", "reverted",
                    "the cost was".

  Consolidation: keep as Entry with importance 1.0; additionally
                 propose a Lesson that captures the avoidance pattern
...
## 3. Loss of function — importance 0.95, never evict
...
the agent can detect and avoid it in future similar refactors.
  ...
  Consolidation: keep as Entry with importance 0.95; if a pattern
                 emerges across multiple loss-of-function entries,
                 promote a Lesson capturing the structural cause.
...
## 9. Self-knowledge — importance 0.95, promote to Lesson, never evict
...
not propagate forward is wasted.
  ...
  Consolidation: keep as Entry with importance 0.95; promote to
                 Lesson with confidence 0.9 and domain_tags
                 including "self-knowledge", "metacognition".
                 Never evict either layer.
...
class MemoryArchive:
    ...
    def evict(self) -> EvictionReport:
        ...
        bounds are exceeded. Honors the rubric's "never evict" rules
        by treating importance >= 0.95 as protected.
        """
        evicted_entries: list[str] = []
        evicted_episodes: list[str] = []
        evicted_lessons: list[str] = []
        ...
        if len(self.working_buffer) > self.working_buffer_max_entries:
            ...
            drop = [
                e for e in sorted_entries[self.working_buffer_max_entries:]
                if e.importance < 0.95  # protect critical
            ]
