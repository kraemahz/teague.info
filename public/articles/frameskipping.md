...
Clippy has no inherhent interest in paper clips. It cares nothing about their existence. Its reward is an integer value. It is trained to maximize that value stored in memory that has a label to outside observers of _num\_paperclips_. Within Clippy's frame of reference this value stored in its memory is what is closest to the reward signal.
