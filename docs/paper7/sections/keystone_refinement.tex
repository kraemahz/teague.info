...
$T = (R, \Xirestrict_{\mathrm{narrow}}, \scope, \testdur,
\damagebound)$ where $\Xirestrict_{\mathrm{narrow}}$ is the
restriction induced by
$\keystonestruct_{\mathrm{narrow}} \prec
\keystonestruct_{\mathrm{current}}$; the test lifts the
\emph{difference}
$\keystonestruct_{\mathrm{current}} \setminus
\keystonestruct_{\mathrm{narrow}}$ while keeping
$\keystonestruct_{\mathrm{narrow}}$ restricted, under the scope and
damage-bound conditions of Definition~\ref{def:cr_test}.
\end{definition}
...
$\Xirestrict = \Xirestrict_{\mathrm{narrow}}$; the scope-bound
construction (Definition~\ref{def:test_scope}) and the
cooperative-closure argument (Theorem~\ref{thm:damage_bound}(c))
...
$\Xirestrict_{\mathrm{narrow}}$ for Mode B).  A test sequence that
has failed to detect $H_0$ under Mode A testing may still succeed
under Mode B testing against a narrower candidate, so the two
modes are not exclusive: restrictions that cannot be fully lifted
...
The nuclear scenario from~\citep[Definition~2,
...
regulatory domains exhibit the same structure.  In each, the
framework's contribution is to formalise the refinement test as a
bounded-damage empirical probe rather than a
moratorium-vs.-wholesale-liberalisation binary.
