...
relaxation injects discrete observation events at scheduled
test times, producing a positive \emph{time-averaged} exogenous
correction rate $\rS^{\mathrm{avg}} > 0$ on the restricted
dimension.  The compositional detail is worth being explicit
about: $\rS$ as defined in~\citep{lasser2026phase} is
...
$\rS$ is upstream notational convenience.  CR delivers a positive
time-average via the discrete log-odds drift
of~\eqref{eq:update_pos}--\eqref{eq:update_neg}, which has
