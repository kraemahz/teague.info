...
\begin{remark}[CR tests the physical mechanism, not just the restriction flag]
\label{rem:physical_mechanism}
A controlled relaxation test does not ask ``what happens if we
remove the restriction?''  It asks the sharper question ``does the
physical pathway that justified the restriction still activate when
exercised?''  The risk-restriction pair $(R, \Xirestrict)$ was
adopted as a bet on a specific causal mechanism---the exercise
pathway $\Pathway$
of~\citep[Definition~2]{lasser2026horizon}---operating from the
capability tuple $(d_1, \ldots, d_k)$ to the predicted
$\volL$-contraction.  Exercising the pathway inside $\scope$ and
observing its detection response is equivalent to probing the
mechanism.  The restriction may be stale not because the original
risk analysis was wrong but because the underlying physical
substrate has changed: new mitigation technology, evolved
materials, different social context, or the intervening decades
of technological progress that make the restriction's original
triggering conditions no longer generic.  The CR machinery
inherits this framing from the pathway definition;
Remark~\ref{rem:scope_existence} names the alternative
(structural resolution) for cases where the mechanism is
mechanically infeasible in the current context.

The test is conducted by actors whose integrity is appropriate to
the domain; the formal construction is indifferent to the
implementation (cryptographic commitment via
\citep{lasser2026exo}, institutional oversight, legal accountability,
or combinations thereof).  Section~\ref{sec:meta_policy} names the
trust apparatus abstractly.
\end{remark}
...
binary hypothesis test} with fixed likelihoods throughout, in the
sequential-analysis tradition of~\citet{wald1947sequential}; the
closest methodological precedent is Bayesian adaptive trial
design~\citep{berry2006bayesian}, which similarly conducts
repeated tests under a bounded patient-risk budget and updates
the posterior on each outcome.  CR generalises this pattern from
clinical trials to the broader class of long-timeline
restrictions, replacing patient safety with the user-set damage
tolerance $\damagebound$ and the Phase~1 cooperative-closure
scope construction.
...
\citet{lasser2026phase} establishes that feedback loop~3
(over-calibrated restriction $\to$ discovery avoidance $\to$
structural ignorance) has a degenerate phase boundary when the
self-correction rate $\rS = 0$ for that
...
relaxation injects discrete observation events at scheduled test
times, producing a positive \emph{time-averaged} exogenous
correction rate $\rS^{\mathrm{avg}} > 0$ on the restricted
dimension.

The compositional detail is worth being explicit about.  $\rS$
as defined in~\citep{lasser2026phase} is
$\inf_t \alpha(t) \cdot c_{\mathrm{S}}$, an infimum over the
trajectory.  Between CR test events the per-step rate
$\alpha(t) \cdot c_{\mathrm{S}}$ on the restricted dimension is
zero, so the inf-form $\rS$ may remain zero even with periodic
testing.  The phase-boundary condition that actually closes the
proof, however, is the uniform-contraction condition on
\emph{time-averaged} log-multiplicative factors
\citep[Theorem~1(a) and Eq.~30]{lasser2026phase}; the inf-form
$\rS$ is upstream notational convenience.

CR delivers a positive time-average via the discrete log-odds
drift of~\eqref{eq:update_pos}--\eqref{eq:update_neg}, which has
nonzero expected magnitude per test under
Assumption~\ref{ass:T1}.  The Wamura degeneracy that
\citep[Proposition~4]{lasser2026phase} characterises is the case
where the rate is identically zero across all $t$ (no exogenous
channel exists at all on the restricted dimension); CR resolves
that degeneracy by creating exogenous channel events, satisfying
the uniform-contraction condition in time-average without
requiring the inf to be positive between scheduled tests.  The
restoring force that lifts Loop~3 off the phase-boundary
degeneracy is therefore the time-averaged drift, not an
instantaneous rate.
