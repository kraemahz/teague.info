...
How do we preserve safety in a regime where agents continuously
discover and escalate new capabilities faster than any generation
can experience their risks through ordinary operation?  The
classical mechanism is catastrophe-as-teacher: a first occurrence
writes historical markers---tsunami stones, disaster records,
institutional memory, building-code revisions, pharmacovigilance
regimes---that subsequent generations consult in lieu of the
experiment they cannot run.  The mechanism has two preconditions.
The first catastrophe must have already been paid for; the markers
must still survive in a form the framework can read.  Neither
condition holds for capabilities that did not exist in previous
generations, which is precisely the regime the framework's own
$\volL$-maximising dynamics generate.  This paper constructs a
formal alternative: a mechanism for generating the evidential
content that historical markers would have recorded, within a
bounded damage budget chosen by the framework rather than the
next generation.

The risk machinery of Goal-Frontier
Maximization~\citep{lasser2026horizon} frames the problem
precisely.  The max-aggregation pathology is identified as the
most important open problem of the sequence: the conservative
max operator, combined with the verification asymmetry inherent
in risk prevention, produces overcaution that is structurally
irresolvable within the existing framework---a risk claim that
successfully prevents catastrophe generates no observation that
could lower the risk estimate, because the very success of the
