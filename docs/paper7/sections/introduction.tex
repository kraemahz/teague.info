...
some of which carry existential risk if pursued without restraint.
...
observation, approaching the over-calibrated-restriction feedback loop
...
lieu of the experiment they cannot run.  Disaster-policy research
has long studied this dynamic under the heading of focusing
events~\citep{birkland1997after}: the policy attention required to
revise restrictions accumulates only after the catastrophe that
made the original restriction look prescient.  In the AI-safety
literature the same structural concern appears as
\emph{safe exploration} (an agent cannot learn the consequences
of an action it never takes~\citep{amodei2016concrete}) and as
the gradual-misalignment scenario of~\citet{christiano2019what},
in which incremental restrictions accumulate without
counter-evidence and the resulting failure looks less like a
catastrophic single event than like the slow erosion of a
society's effective optionality.  The precautionary-principle
literature in policy and law makes the same structural
tradeoff explicit: precaution against unfamiliar harms is
appropriate but accumulates costs the precaution itself renders
invisible~\citep{sunstein2005laws}.  This paper formalises the
structural problem common to these literatures and constructs a
mechanism for generating the evidential content historical
markers would have recorded, within a bounded damage budget
chosen by the framework rather than the next generation.
...
A practical asymmetry across these domains is worth naming
explicitly.  Three of the four (chemical contamination, nuclear
dual-use, pandemic preparedness) sit on top of pre-existing
institutional infrastructure built for exactly this kind of
bounded empirical test: the EPA, the IAEA, biosafety committees
and BSL-tiered containment.  The fourth, AI capability
restriction, has no equivalent infrastructure for the
bounded empirical testing this protocol assumes.  The
protocol's deployability in the AI case is therefore conditional
on institutional development that this paper does not provide and
does not estimate the cost of.  We claim the formal apparatus, not
the institutional readiness; the gap between the two is
load-bearing for the AI application and should be acknowledged
rather than elided.
...
is collected in Appendix~\ref{app:dependencies}.
