...
The risk machinery of Goal-Frontier
Maximization~\citep{lasser2026horizon} identifies the
max-aggregation pathology as the most important open problem the
paper names.  The conservative max operator, combined with the
verification asymmetry inherent in risk prevention, produces
overcaution that is structurally irresolvable within the existing
framework: a risk claim that successfully prevents catastrophe
generates no observation that could lower the risk estimate, because
the very success of the restriction is what prevents the
counterfactual test.

This is a distinct problem from the compound feedback loops analysed
in~\citep{lasser2026phase}.  That work asks: when does partial
subsumption self-correct versus cascade?  The present paper asks:
when the framework correctly restricts a risky capability, how does
it ever learn that the restriction was unnecessary?  The two
problems reinforce each other:~\citep[Proposition~4]{lasser2026phase}
shows the restriction-verification gap is a degenerate phase
boundary with zero self-correction rate ($\rS = 0$), but the phase
boundary machinery does not by itself supply the missing
...
The Fudai case is motivating rather than directly soluble by the
construction of this paper.  At Fudai, historical evidence existed:
ancient tsunami stones recorded the relevant inundation heights.
Modern decision-makers declined to consult that evidence, preferring
the narrower observational window of recent decades.  Wamura's
contribution was to restore the historical record to the decision
process; no experimental mechanism was needed.  Controlled
relaxation addresses the complementary case: when no such historical
record exists, or when the record is itself contested, the framework
must generate evidence experimentally.
...
The situation forces the sharper question.  Historically, the
long-timeline-risk problem has been solved by catastrophe-as-teacher:
a first occurrence generates historical markers
(tsunami stones, contamination memorials, disaster records,
institutional memory) that subsequent generations can consult in lieu
of the experiment they are unable to run.  This mechanism is
available only if the first catastrophe has already been paid for
and the markers still survive in a form the framework can read.
Neither condition holds for capabilities that did not exist in
previous generations.  In a regime where agents continuously
escalate new capabilities---by design, under the framework's own
$\volL$-maximising dynamics---the technological cadence outpaces the
accumulation of historical markers.  For a novel capability discovered
at time $t_0$ with realisation timeline $T$, markers from prior
instances do not exist if no prior instance occurred; and waiting
for the catastrophe that would produce them is the failure mode the
framework's restriction regime is meant to prevent.  Controlled
relaxation is the construction that generates the evidential content
historical markers would have recorded, without paying the cost that
