...
Assumptions~\ref{ass:T1}--\ref{ass:T5} with
$p_{\mathrm{ex}} = 1$ (the full-exercise regime of
Assumption~\ref{ass:T5}, enforced by scope construction in
Phase~1):
\begin{itemize}
...
\item[(c)] \textbf{Rate under $H_1$ (pathway dangerous).}  Each
  ...
  $(1 - \beta) \gg \alphafp$.  After a single $O = 1$
  observation, the posterior is exactly
  $\pR(t_1) = \pR(0) \Lone / (\pR(0) \Lone + (1 - \pR(0)))$,
  so a single detection suffices to push $\pR$ above $1 - \delta$
  for any $\delta > \delta_{\min}$, where
  $\delta_{\min} = (1 - \pR(0)) / (\pR(0) \Lone + (1 - \pR(0)))$.
  For $\thetaone$ close to $1$, small $\beta$, and modest
  $\pR(0)$, $\delta_{\min}$ is small (e.g.,
  $\pR(0) = 0.5$, $\Lone = 100$ gives $\delta_{\min} \approx
  0.02$); for tighter $\delta$ requirements, the expected number
  of tests grows as $O\bigl(1 / [\thetaone (1 - \beta)]\bigr)$.
...
numbers~\citep[Ch.~6]{cover2006elements} (or the martingale
convergence theorem of~\citet{doob1953stochastic}, which gives
the same conclusion under weaker independence assumptions) gives
...
Sensitivity of $N_{\mathrm{clear}}$ to the model parameters
(computed from~\eqref{eq:n_clear} with $\thetaone = 1$
throughout):
...
\toprule
$\beta$ & $\alphafp$ & $\pR(0)$ & $p_{\mathrm{clear}}$ & $\Lzero$ & $N_{\mathrm{clear}}$ \\
\midrule
0.10 & 0.01 & 0.50 & 0.01 & 0.101 & 3 \\
0.10 & 0.01 & 0.70 & 0.01 & 0.101 & 3 \\
0.10 & 0.01 & 0.90 & 0.01 & 0.101 & 3 \\
0.01 & 0.01 & 0.50 & 0.01 & 0.0101 & 1 \\
0.30 & 0.01 & 0.50 & 0.01 & 0.303 & 4 \\
0.50 & 0.01 & 0.50 & 0.01 & 0.505 & 7 \\
0.10 & 0.05 & 0.50 & 0.01 & 0.105 & 3 \\
0.10 & 0.20 & 0.50 & 0.01 & 0.125 & 3 \\
\bottomrule
\end{tabular}
\end{center}

The detection-sensitivity parameter $\beta$ dominates the count;
the false-positive rate $\alphafp$ contributes weakly until
$\alphafp \to (1-\beta)$, where the likelihood ratio $\Lzero$
approaches $1$ and convergence stalls.  For $\thetaone < 1$
(realistic activation rates below the maximum), $\Lzero$
increases toward $1$ proportionally to $1 - \thetaone$, and
$N_{\mathrm{clear}}$ grows accordingly.
\end{remark}

\begin{remark}[Robustness to $p_{\mathrm{ex}}$ misestimation]
\label{rem:pex_robustness}
The convergence rate of Theorem~\ref{thm:convergence} is stated
under the full-exercise regime of Assumption~\ref{ass:T5}
($p_{\mathrm{ex}} = 1$), with the partial-exercise variant
discounted by a known $p_{\mathrm{ex}} \in (0, 1]$.  In practice
the test designer cannot directly observe $p_{\mathrm{ex}}$; it
depends on environmental contingencies during the test window
and may be inferable only after the fact.  A robust variant
uses a conservative lower bound $p_{\mathrm{ex}}^{\min} \leq
p_{\mathrm{ex}}$, yielding likelihoods $\Lzero^{\mathrm{cons}}
\geq \Lzero$ and $\Lone^{\mathrm{cons}} \leq \Lone$ that
under-state the evidence in the convergence-conservative
direction.  Under Assumption~\ref{ass:T1} applied to the
conservative likelihoods (the modeled $\Lzero^{\mathrm{cons}}
< 1 < \Lone^{\mathrm{cons}}$), the convergent hypothesis is
unchanged and the rate is bounded below by $\KL$ computed with
the conservative likelihoods.  The qualitative fact that the
test was conducted within the scope is sufficient for
convergence; only the rate degrades under under-estimation of
$p_{\mathrm{ex}}$.  The misspecification regime in which the
convergent hypothesis flips is delineated in
Section~\ref{sec:discussion}, Limitations.

The dangerous failure mode is in the opposite direction:
upward misspecification of $p_{\mathrm{ex}}$.  If the modeller
assumes $p_{\mathrm{ex}} = 0.4$ but the true value is $0.05$,
most tests produce non-exercise null events, and a meta-policy
that uses the modelled likelihoods will conclude the capability
is cleared when the test scope never actually activated the
pathway.  Upward misspecification is a detectable failure mode:
the observed exercise frequency over a sequence of tests will
fall systematically below the modelled $p_{\mathrm{ex}}$, and
the meta-policy of Section~\ref{sec:meta_policy} must treat
that gap as a test-design failure rather than a cleared claim.
Operationally, the meta-policy should track an empirical
$\hat{p}_{\mathrm{ex}}$ from observed exercise events and
reject the test sequence when $\hat{p}_{\mathrm{ex}}$ falls
below the modelled $p_{\mathrm{ex}}$ by a configured
tolerance, returning the restriction to its prior state with
the test outcomes discarded.
\end{remark}
