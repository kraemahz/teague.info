...
\item[(c)] \textbf{Rate under $H_1$ (pathway dangerous).}  Each
  ...
  0.01$); for tighter $\delta$ requirements, the expected number
...
pathway.  Upward misspecification is a detectable failure mode: the
observed exercise frequency over a sequence of tests falls
systematically below the modelled $p_{\mathrm{ex}}$.  The
meta-policy of Section~\ref{sec:meta_policy} treats that gap as
a test-design failure (Definition~\ref{def:test_initiation},
criterion~(v)): the running $\hat{p}_{\mathrm{ex}}$ is tracked
against the modelled $p_{\mathrm{ex}}$ and the test sequence is
rejected when the gap exceeds a configured tolerance.
\end{remark}
