...
touching $d$.  Hence the exercise probability of $d$'s pathways
is unchanged during the test (by the locality property of
pathway definition
in~\citep[Definition~2]{lasser2026horizon}).  For
...
tuple).  Enumerating the cooperative surface
$\partial \scope$ and forward-reachability chains is polynomial
in $|P|$ for fixed depth.  Taking the maximum across enumerated
pathways and adding cooperative-surface terms yields
$\maxcon(\scope)$ in $O(|\scope|^L \cdot |\scope|) =
O(|\scope|^{L+1})$ time, which matches the asserted
...
Assumption~\ref{ass:T2}, the likelihoods in the definition of
$\ell_n$ match the true data-generating distribution.  Under
...
logistic map ($\pR = \sigma(\logit \pR)$),
