...
touching $d$.  Since a pathway's exercise probability
\citep[Definition~2]{lasser2026horizon} is a function of its
prerequisite set and intermediate transitions, and the test
modifies only capability-availability within $\scope$ (the
only capabilities that change status during the test), the
...
forward-reachability chains is $O(|P|^L)$.  Taking the maximum
across enumerated pathways and adding cooperative-surface
terms yields $\maxcon(\scope)$ in $O(|\scope|^L \cdot |\scope|
+ |P|^L) = O(|P|^{L+1})$ time, which matches the asserted
$O(|\scope|^L)$ bound up to the bounded pathway-evaluation
factor and the cooperative-surface contribution.  When the
cooperative forward-reachability depth $L'$ exceeds $L$, the
bound becomes $O(|P|^{\max(L+1, L')})$.
...
$\testmix = 1$, upper-bound if $\testmix > 1$).  Each
opportunity misses with probability $1 - \deltaagg$.  Hence
