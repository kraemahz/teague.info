#!/usr/bin/env bash
# Build the Distill HTML version of Paper 7.
# Usage: ./build-web.sh
# Output: ../../public/papers/wamura/index.html
...
OUT_DIR="$SCRIPT_DIR/../../public/papers/wamura"
...
# Resolve cross-references for Paper 7
# Numbers from main.aux after pdflatex build.
ref_map = {
    # Sections
    'sec:introduction': '1',
    'sec:framing_precondition': '1.2',
    'sec:real_world': '1.4',
    'sec:not_do': '1.5',
    'sec:verification_asymmetry': '2',
    'sec:relaxation_protocol': '3',
    'sec:connection_phase_boundary': '3.3',
    'sec:damage_guarantees': '4',
    'sec:detection_sensitivity': '5',
    'sec:convergence': '6',
    'sec:meta_policy': '7',
    'sec:worked_example': '8',
    'sec:keystone_refinement': '9',
    'sec:nuclear_example': '9.3',
    'sec:integration': '10',
    'sec:discussion': '11',
    # Appendices
    'app:proofs': 'A',
    'app:proof_asym_evidence': 'A.1',
    'app:proof_damage_bound': 'A.3',
...
{
  "title": "Controlled Relaxation Protocols for the Wamura Problem",
  "description": "Formalizes the Wamura problem — restrictions justified by long-timeline risks cannot be revised through observation because the evidence that would revise them cannot arrive within any practical decision cycle. Introduces controlled relaxation protocols: bounded, monitored exercise windows that generate Bayesian evidence about risk claims while provably bounding maximum damage. Extends to keystone-refinement tests for the misconfigured-restriction case.",
  "authors": [
    {
...
<d-title>
  <h1>Controlled Relaxation Protocols for the Wamura Problem</h1>
  <p style="margin-top: 0.5em;"><a href="/papers/wamura/wamura.pdf" style="color: #666; text-decoration: none; border-bottom: 1px solid #ccc;">📄 Download PDF version</a></p>
</d-title>
...
if [ -f "$PAPER_DIR/main.pdf" ]; then
    cp "$PAPER_DIR/main.pdf" "$OUT_DIR/wamura.pdf"
    echo "PDF copied to $OUT_DIR/wamura.pdf"
fi
