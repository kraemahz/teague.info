...
The foundational transfer result on which the remainder of the paper builds:
...
Let $\volL$ satisfy axioms M1--M6 (Proposition~1 of
    \citet{lasser2026poset}).\footnote{The poset paper's M4 uses
    poset-disjointness (a stronger condition than the foundational paper's
    set-disjointness). The transfer holds because on a poset with
    non-negative independent weights, removing nodes from $\dc G$ can only
    reduce the sum, regardless of whether M4-additivity holds for the
    specific partition used in the foundational proofs.}
...
is a property of the \emph{objective}, not of the \emph{planner}.
