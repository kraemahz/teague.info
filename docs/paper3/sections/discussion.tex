...
\paragraph{Mixed strategies and partial subsumption.}
The trajectory comparison models a binary choice: full domination versus
full diversity maintenance. A more sophisticated coalition could pursue
partial subsumption: absorb low-leverage non-members (whose
$r_{\mathrm{ext}}$ contribution is negligible) while preserving
high-leverage ones. This mixed strategy captures some $\Delta_0$ while
retaining most $r_{\mathrm{ext}}$, and likely dominates both pure
strategies for a wide parameter range. The binary framing overstates the
diversity advantage relative to the optimal mixed strategy. Characterizing
the optimal partial-subsumption policy is an open problem.
...
overcaution but increase the probability of missing real threats. Additionally, during bootstrap when $T_j^{\mathrm{risk}}$ starts at a
