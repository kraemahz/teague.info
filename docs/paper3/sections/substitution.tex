...
objective, the convergence runs in the opposite direction.

This section shows that the combined machinery of the discounted objective
...
\citet{lasser2026poset}) addresses the substitution problem under a diversity condition that holds
in the regime where the coalition has not yet achieved the capability to
internally replicate all external discovery. The protection degrades as
...
\begin{corollary}[Observational Individuation Restores Static Protection]
...
Let $(G, \C)$ be a population state with coalition $K$, discount factor
    $\gamma \in (0, 1)$, and planning horizon $H$. Assume:
\begin{itemize}
...
Then:
\begin{enumerate}
\item[\textbf{(a)}] \textbf{Individual growth-rate collapse.} In a fully
    dominant state (Definition~\ref{def:dominant}), the individual-capability
    component of $r_{\mathrm{ext}}$ is zero: no non-member's individual
    capabilities produce novel poset nodes, since they are all already in
    $G_K^{\mathrm{ind}}$. Novel cooperative capabilities from cross-coalition
    interaction may persist (Section~\ref{sec:substitution_coop}), providing
    additional protection beyond this bound.
\item[\textbf{(b)}] \textbf{Trajectory comparison.} The actor at time
    $t = 0$ compares two strategies: \emph{dominate} (subsume remaining
    non-member capabilities, gaining a one-time $\volL$-expansion
    $\Delta_0 \geq 0$, then grow at internal rate $r_K$ per step) versus
...
\gamma^* = 1 - \frac{r_{\mathrm{ext}}}{\Delta_0 - r_K}
    = \frac{\Delta_0 - r_K - r_{\mathrm{ext}}}{\Delta_0 - r_K}
\end{equation}
    Three cases arise. If $\Delta_0 \leq r_K$ (the domination bonus does
    not exceed one step of internal growth), diversity dominates for all
    $\gamma > 0$. If $r_K < \Delta_0 \leq r_K + r_{\mathrm{ext}}$, then
    $\gamma^* \leq 0$, and diversity again dominates unconditionally. Only
    when $\Delta_0 > r_K + r_{\mathrm{ext}}$ (the domination bonus exceeds
    both internal growth and external contribution) does a non-trivial
    threshold $\gamma^* \in (0, 1)$ exist. In all cases, since
...
(b) Under the linearized model, each strategy produces a constant per-step
$\Delta \volL$. The domination strategy gains $\Delta_0$ at $t = 0$ (the
one-time subsumption bonus) and $r_K$ per step thereafter. The diversity
strategy gains $r_K + r_{\mathrm{ext}}$ per step from $t = 0$:
\begin{align*}
\Vdisc^{D} &= \Delta_0 + \sum_{t=1}^{\infty} \gamma^t \cdot r_K
  = \Delta_0 + \frac{r_K \cdot \gamma}{1 - \gamma} \\
...
If $\Delta_0 \leq r_K$, the left side is positive for all $\gamma > 0$.
If $\Delta_0 > r_K$, rearranging gives $\gamma > 1 -
r_{\mathrm{ext}} / (\Delta_0 - r_K) = \gamma^*$. When $\Delta_0 - r_K
\leq r_{\mathrm{ext}}$, $\gamma^* \leq 0$ and the condition holds for all
$\gamma > 0$. When $\Delta_0 - r_K > r_{\mathrm{ext}}$, $\gamma^* \in
(0, 1)$ and a sufficiently long planning horizon makes diversity strictly
preferable.
...
\label{cor:gradual}
Under the gradual domination model (subsumption over $\tau_D$ steps at rate
    $\delta = \Delta_0 / \tau_D$, with $r_{\mathrm{ext}}$ lost during
    domination), the discounted value difference is:
\begin{equation}
\label{eq:gradual}
\Vdisc^{\mathrm{div}} - \Vdisc^{D}
...
two protections are complementary but not redundant: observational
...
fully dominant state with $|K| = 1$. Two layers of protection apply. First,
observational individuation (Corollary~\ref{cor:a1_from_oi}) makes each
step of the domination process $\volL$-contracting: every distinguishable
agent eliminated costs at least 1 bit. Second, if the population satisfies
(A1), the dynamic argument (Proposition~\ref{prop:anti_monopolar} and
Corollary~\ref{cor:gradual}) shows that the diversity trajectory dominates
the domination trajectory over the planning horizon. Neither protection is
a safety constraint imposed from outside the objective. Both emerge from
$\volL$-maximization itself. The result is structurally analogous to
...
\paragraph{Scope of the two protections.} Observational individuation
...
dynamics that observational individuation does not discharge.
