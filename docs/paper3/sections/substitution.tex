...
singleton superintelligence scenario: a coalition so capable that no other
agent's existence contributes to the objective.
...
\citet{lasser2026poset}) addresses the substitution problem under a
diversity condition that holds generically for populations with
...
requires ongoing novel capability contributions, but it restores the
per-elimination cost that the substitution problem appeared to dissolve.
