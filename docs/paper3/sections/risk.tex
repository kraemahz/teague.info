...
branches with large negative $\Delta \volL$.
...
require $d_j$ is present under both sequences and cancels. The value of
restriction exceeds the value of inaction when
$-|\Delta \volL| > -\Risk_j$, i.e., when $\Risk_j > |\Delta \volL|$.
\end{proof}

\paragraph{Connection to Lemma 2.} The foundational paper's
Lemma~2 says restriction is always $\volL$-contracting in isolation. The
discounted objective adds the temporal dimension: the immediate contraction
is weighed against the avoided future contraction. The preemptive restriction
criterion is the quantitative version of the quarantine pattern from
Proposition~1(b) of \citet{lasser2026gfm}, now with an explicit threshold
