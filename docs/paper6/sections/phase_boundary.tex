...
agents), which holds generically for finite action sets (see proof).  A computable lower
...
smoothly, which means the destabilizing cascade has discrete tipping
...
agent-channel graph (each subsumption removes channels covering at
...
\rS \cdot \rhomincross + \alphamin \cdot c_{\mathrm{V}} \cdot \underline{\nu}
  ...
  + \dmax \cdot \frac{\Delta\rho_{\mathrm{avg}}}{\Lyap_{\mathrm{avg}}}
\end{equation}
Below this surface (total correction---exogenous channels plus
endogenous incentive---dominates both error-proportional growth and
drift exposure): Part~(a)'s contraction condition is satisfied,
so $\Lyap$ converges to the bounded neighborhood of
Equation~\ref{eq:neighborhood}.  The system is in the
self-correcting basin provided additionally that $\Lyap_\infty <
\epsilon_{\mathrm{safe}}$ (i.e., the neighborhood is small enough for
policy correctness).  Above this surface, the contraction condition
fails; the system enters the absorbing basin (under B1) or the
metastable basin (under B1$'$) if the additional blindness and
threshold-trigger conditions of Parts~(b)/(b$'$) are also met.
...
$\alphamin \cdot c_{\mathrm{V}} \cdot \underline{\nu}$ alone exceeds
the right-hand
side, $(\rhomincross)^* \leq 0$ and endogenous correction suffices
without any cross-substrate channels.  Setting $c_{\mathrm{V}} = 0$
recovers the purely exogenous formula $(\rhomincross)^* =
\mathrm{RHS} / \rS$.  The $\dmax$ term raises the threshold when
subsumption removes channels covering well-calibrated dimensions
(high $\Delta\rho_{\mathrm{avg}} / \Lyap_{\mathrm{avg}}$).
\end{theorem}
...
non-subsumption steps).  The second bounds the additive drift
\emph{per step} (not merely in average): since
$b_t = \nsub(t) \cdot \dmax \cdot \Delta\rho(t)$ and both $\dmax$
and $\Delta\rho(t)$ are bounded per step by construction
(C2$'$; $\Delta\rho(t)$ is the redundancy loss at a single step,
bounded by the total redundancy of the poset), the per-step bound
$\bar{b} = \dmax \cdot \max_t \Delta\rho(t)$ holds.  The per-step
bound is needed (rather than the average bound) because the
affine-recursion convergence theorem requires controlling the
weighted suffix sums $\sum_s b_s \prod_{t>s} a_t$, where large
individual $b_s$ terms are not averaged away by the product tail.

Note that $a_t > 0$ for all $t$: the step-size condition
(Section~\ref{sec:lyapunov}) ensures
$1 - \alpha(t) \cdot (c_{\mathrm{S}} \cdot \rhomincross(P_t)
+ c_{\mathrm{V}} \cdot \nu(P_t, \Wm_t)) > 0$, i.e.,
$1 - \rS(t) \cdot \rhomincross(P_t) - \alpha(t) \cdot c_{\mathrm{V}}
\cdot \nu(P_t, \Wm_t) > 0$.  On non-subsumption steps $a_t$ equals
this quantity.  On subsumption steps, $a_t$ adds $\rW(t) \geq 0$,
so $a_t \geq 1 - \rS(t) \cdot \rhomincross(P_t) - \alpha(t) \cdot
c_{\mathrm{V}} \cdot \nu(P_t, \Wm_t) > 0$.
...
\limsup_{t \to \infty} \Lyap(\Wm_t)
  \leq \mu \cdot \frac{\bar{b}}{1 - \bar{a}}
\end{equation}
where $\mu \geq 1$ is the \emph{overshoot constant} accounting for
transient excursions during subsumption bursts (steps where
$a_t > 1$).  Since $a_t$ is bounded above (by
$1 + \rW$ on subsumption steps) and the log-average is strictly
negative, $\mu$ is finite and depends on the ratio
$\max_t a_t / \bar{a}$; see~\citet{liberzon2003switching},
Theorem~3.1 for the precise form.
In the neighborhood radius
(Equation~\ref{eq:neighborhood}) and throughout the linearized
analysis, we absorb $\mu$ into the constants by writing
$\Lyap_\infty \leq \bar{b}/(1 - \bar{a})$, which is valid in the
linearized regime where $|a_t - 1| \ll 1$ (so $\mu \approx 1$)
but should be read as carrying an implicit overshoot factor outside
that regime.

The abstract bound depends on the
...
\alphamin \cdot c_{\mathrm{V}} \cdot \underline{\nu} - \rW \cdot \rsub$, yielding
Equation~\ref{eq:neighborhood}.  Outside
...
differ from these closed-form expressions.
...
(Definition~\ref{def:transition}) and the optimal action at $\Wm^*$ is
unique (i.e., $G_j(\Wm^*) > 0$ for the optimal action---a
\emph{strict} gap, which holds generically for finite action sets and
fails only on a measure-zero set of parameters), each action gap
$G_j(\Wm)$ is continuous with $G_j(\Wm^*) > 0$.  Therefore the set
...
partial derivatives evaluated at their sup over the ball
$\{\Wm : \Lyap(\Wm) \leq \Lyap_\infty\}$ (i.e., the neighborhood
from Equation~\ref{eq:neighborhood}).  If a global Lipschitz
constant $L_G$ for the action gaps is available, $\|\nabla_\epsilon
G_j\|_w$ can be replaced by $L_G$, eliminating the dependence on
...
smallest error on dimension $k$ that flips any pairwise
comparison for a preserve-optimal agent, where
$\mathcal{A}_{t^*}^+ = \{j : G_j(\Wm^*) > 0\}$).  The per-coordinate quantity $\min_k w_k \cdot
