...
\Lyap_\infty \leq \frac{\rsub \cdot \dmax \cdot
  \overline{\Delta\rho}}{\rSeff
  \cdot \rhomincross - \rW \cdot \rsub}
\end{equation}
...
\end{equation}
where $\rSeff = \rS + c_{\mathrm{V}} \cdot \underline{\nu} /
\rhomincross$ is the effective correction rate combining exogenous
channels ($\rS$) with the endogenous calibration incentive, and
$\underline{\nu} = \inf_t \nu(P_t, \Wm_t)$ is the worst-case
$\volL$-sensitivity along the trajectory.  (The per-step bound from
Proposition~\ref{prop:error_dynamics} carries the time-varying
$\alpha(t) \cdot c_{\mathrm{V}} \cdot \nu(P_t, \Wm_t)$; the
simplified condition uses the inf bound.)  The endogenous term widens
the self-correcting basin: the actor's own $\volL$-gradient toward
better calibration supplements the observation channels, making the
condition easier to satisfy than the purely exogenous version
($\rS \cdot \rhomincross > \ldots$).  Setting $c_{\mathrm{V}} = 0$
recovers the purely exogenous condition.
...
$(\rSeff, \rW, \rsub, \dmax, \Delta\rho_{\mathrm{avg}},
\Lyap_{\mathrm{avg}})$ is:
\begin{equation}
...
The endogenous term ($c_{\mathrm{V}} \cdot \underline{\nu}$) in
$\rSeff$ lowers the threshold: the actor's own calibration incentive
supplements the channels, reducing the redundancy needed.  The
$\dmax$ term raises the threshold when subsumption removes channels
covering well-calibrated dimensions (high
...
\overline{\Delta\rho}$ and $1 - \bar{a} = \rSeff \cdot \rhomincross -
