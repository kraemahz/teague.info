...
gating, causal attribution, behavioral residual, SPRT
detection~\citep{wald1947sequential}, correlational co-occurrence)
are each instances of $c_i$ with specific
...
The sensitivity-weighted definition is strictly more informative than
a raw channel count.  A channel with $\sigma \ll \sigma_{\mathrm{thr}}$
contributes negligibly to $\rho_k$.  This correctly diagnoses
skeleton-substrate strategies where channels are nominally present
...
Section~\ref{sec:lyapunov} relies on this additivity: the chain
is joint factorization $\Rightarrow$ independence $\Rightarrow$
additive correction signals.  The phase boundary
