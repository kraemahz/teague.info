...
controlled relaxation protocol is the \emph{mechanism} that makes
$\rS$ positive for Loop~3, without which $\rS = 0$ and the
self-correcting sufficient condition fails for every configuration
(Proposition~\ref{prop:wamura}).  Absorption or metastability on the
risk dimension still requires the Part~(b)/(b$'$) blindness and
threshold-trigger conditions; the degeneracy is that the safe side
of the inequality is empty, not that every state is absorbing without
...
non-stationarity on the restricted dimension.  All three conditions
of Part~(b) are therefore met, and the cascade drives the system
toward maximal restriction.
