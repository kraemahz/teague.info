...
($c_{\mathrm{V}} \cdot \nu = 0$, because the actor cannot estimate
$\nu$ on a dimension where exercise is prohibited), the phase boundary
collapses: every non-trivial risk restriction is absorbing, and the
framework converges to maximal restriction.
\end{proposition}

\begin{proof}
With $\rS = 0$ and $c_{\mathrm{V}} \cdot \nu = 0$ (the actor cannot
estimate the $\volL$-sensitivity on an unexercised dimension), the
cumulative error budget $B(t_0, T)$ becomes
\begin{equation*}
