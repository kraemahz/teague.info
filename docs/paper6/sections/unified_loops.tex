...
(Strictly, $\nu$ in C1 is an objective marginal sensitivity, not an
actor-estimated quantity.  The argument here is that the endogenous
correction mechanism \emph{operates through} the actor's
$\volL$-gradient computation; when the actor cannot estimate $\nu$,
the correction channel it would drive is inactive regardless of the
objective value of $\nu$.  The $c_{\mathrm{V}} \cdot \nu = 0$
condition is thus a statement about the \emph{effective} correction
rate, not about the objective sensitivity.)
\begin{equation*}
