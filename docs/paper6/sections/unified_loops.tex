...
Papers~1--5 are instances of the same $(P, \Wm)$ dynamical system with
...
\item $\rS$: the rate at which Paper~3's structural discovery
  value (Proposition~2, Definition~5) drives the actor toward
  learning structure.
\end{itemize}

\paragraph{Phase boundary specialization.}
Self-correction dominates when the information value $I(d_1 \leq d_2)$
(Paper~3, Definition~5) exceeds the immediate $\volL$ contraction from
...
parameters from the general phase boundary.  This formalizes Paper~2's
discussion of structural avoidance as a phase-boundary condition
...
$\Delta\rho(t) > 0$ or $\Lyap(\Wm_t) > 0$), $B > 0$ and Part~(b)
of Theorem~\ref{thm:phase_boundary} applies.  Every restriction step
reduces observation (no exercise produces no evidence), increasing
world-model error on the risk dimension, making the next restriction
appear more justified under the miscalibrated model.

This is the formal statement of why the Wamura problem was called
``the most important open problem'' in Paper~3's discussion: without a
...
1. Structural avoidance & Known poset & Leverage estimates &
  Discovery value (P3 Prop.\ 2) & Un-learned structure \\
