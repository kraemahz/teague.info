...
competitors.  Here the convergence is measured against a
...
path to that state is self-correcting or absorbing.  Self-correcting
means the coalition detects its error through observation-channel
feedback and reverses course.  Absorbing means each step makes the
next step's error less detectable, until the monopolar state becomes a
fixed point from which no locally rational trajectory escapes.
...
harm less detectable.  This form is closely related to the ``what
failure looks like'' scenario of~\citet{christiano2019what}, in
which AI systems gradually become less aligned through feedback-loop
...
\item \textbf{The phase boundary theorem}
  ...
  The central result is a characterized boundary in terms of the
  ...
  $\rhomincross$.  When correction exceeds degradation (the safe
  side of the inequality), the Lyapunov function $\Lyap(\Wm_t)$
  converges to a neighborhood of zero whose radius shrinks as
  subsumption ceases, and the anti-monopolar property applies once
  the trajectory enters that neighborhood.  When degradation exceeds
  correction, the system enters an absorbing basin (under full
  blindness) or a metastable basin (under partial indirect evidence).
