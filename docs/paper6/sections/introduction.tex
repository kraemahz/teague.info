...
Paper~3~\citep{lasser2026horizon} establishes the anti-monopolar
property: under discounting, full capability domination by a single
substrate is anti-maximizing when the discount factor $\gamma$ exceeds
a threshold $\gamma^* = 1 - \rext / \Delta_0$ (Proposition~6,
``Domination is Anti-Maximizing'').  But
...
identified in the gap analysis across Papers~1--5 and share a common
dynamical form: a locally rational decision narrows the observation
basis, making the \emph{next} locally rational decision's harm less
detectable.
...
reinforcing the avoidance.  Paper~3's structural discovery
...
\item \textbf{The phase boundary theorem}
  ...
  self-correction rate $\rS$, the error-proportional degradation rate
  $\rW$, the subsumption frequency $\rsub$, the exogenous drift
  rate $\dmax$ from lost channels, and the minimum cross-substrate
  redundancy $\rhomincross$.  Below the boundary, the
...
\item \textbf{Minimum channel redundancy}
  ...
  the criterion whenever $\Kmin > 1$ and leaves zero margin otherwise).
\end{enumerate}
...
enforcement mechanism for channel integrity is
Paper~5's~\citep{lasser2026exo} cryptographic commitment protocol.
The combination is: Paper~5 ensures channels are honest; this paper
ensures enough honest channels survive.

\subsection{Dependencies on prior papers}

Table~\ref{tab:dependencies} summarizes which results from
Papers~1--5 this paper relies on and how they enter the analysis.
...
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Paper} & \textbf{Result used} & \textbf{Role here} \\
\midrule
P1 & Self-trust $T_s$ (Appendix B) & Restoring force: self-correction via prediction residuals \\
P2 & Axiom verification (Prop.\ 1), leverage & Properties of $\volL$ the dynamics preserves or violates \\
P3 & Anti-monopolar property (Prop.\ 6, ``Domination is Anti-Maximizing'') & Starting point: the linearized growth model we extend \\
P3 & Structural discovery value (Prop.\ 2) & Self-correction force in feedback loop 1 \\
P3 & Substrate physics (Defs.\ 10, 12--13) & Substrate partition the phase boundary is defined over \\
P4 & Multi-channel architecture (Table~1) & Instances of observation channels \\
P4 & Risk-trust $L^2$ convergence (Prop.\ 2) & Template for bounding correction rates \\
P5 & Channel isolation \& independence & Integrity conditions on observation channels \\
\bottomrule
\end{tabular}
\caption{Dependencies on Papers~1--5.}
