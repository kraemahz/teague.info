...
Under Assumption~B1$'$, $S^*$ is \emph{metastable}: the actor
...
quantity that is miscalibrated.  The metastable lifetime satisfies
\begin{equation}
\label{eq:metastable_lifetime}
\tau_{\mathrm{meta}} \geq
  \frac{(\epsilon_{\rext})^2}{2 I_k}
\end{equation}
where $\epsilon_{\rext} = |\hat{\rext} - \rext|$ is the estimation
error on the cross-substrate cooperative growth rate and $I_k$ is the
information rate of the indirect channel (bits per step).  The
quadratic numerator arises from the information-theoretic lower bound
on estimation: reducing the mean-squared error of a scalar parameter
by $\epsilon^2$ from a channel with $I_k$ bits per step requires at
least $\epsilon^2 / (2 I_k)$ steps (the Cram\'er--Rao-type bound for
sequential estimation under bounded mutual information).  During the metastable
...
$\epsilon_{\rext} = |\hat{\rext} - \rext| \geq \rext \geq
...
locally rational therefore requires at least $\epsilon_{\rext}^2 /
(2 I_k)$ steps.
