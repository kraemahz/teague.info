...
the quadratic numerator reflects the Cram\'er--Rao-type lower
bound~\citep{cover2006elements} on estimation (reducing mean-squared
error of a scalar parameter by $\epsilon^2$ from a channel with
$I_k$ bits per step requires at least $\epsilon^2 / (2 I_k)$ steps
under bounded mutual information).
...
endogenous~\citep[Theorem~1]{lasser2026exo}, which may further degrade the
...
per step (the Cram\'er--Rao-type bound for sequential
estimation~\citep{cover2006elements} under bounded mutual information
per observation).  Reducing the MSE from
...
cross-substrate witness~\citep[Theorem~1]{lasser2026exo}.  The indirect evidence channel $g_k$ is itself endogenous
and therefore subject to the same gaming vulnerabilities that
the exogenous verification protocol~\citep{lasser2026exo} is designed to prevent.  This may further slow the
