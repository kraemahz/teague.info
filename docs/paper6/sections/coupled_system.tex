...
the poset axioms M1--M6~\citep{lasser2026poset}) and $\mathcal{W} = [0,1]^K$.
\end{definition}
...
The action set $\mathcal{A}_t$ is finite at each step (consisting of
preserve/subsume decisions for each agent pair; the finiteness of the
agent set implies finiteness of $\mathcal{A}_t$).  At each step, the
actor selects the action $\pi_t \in \mathcal{A}_t$ that maximizes its
...
the horizon framework~\citep{lasser2026horizon}~(Definition~1) and $H$ is the planning horizon.  The actor
uses the discounted objective $\Vdisc$~\citep{lasser2026horizon} but evaluates it under
...
agents still present.  Subsumption removes agents, which removes
their observation channels.  This in turn degrades $\Wm$ on
dimensions those channels served.

\begin{remark}[Myopic vs.\ discounted]
\label{rem:myopic}
The discounted formulation is necessary for consistency with the
anti-monopolar result~\citep[Proposition~6]{lasser2026horizon}, which
compares discounted trajectory values.  A myopic (one-step) variant of
the phase boundary exists---replace $\Vdisc$ with one-step
$\Delta\volL$---but produces strictly weaker results.  It cannot
invoke the $\gamma^*$ threshold~\citep{lasser2026horizon}, and the
self-correction guarantee applies only to immediate decisions, not
trajectory-level convergence.  We analyze the discounted case
throughout and note where the myopic specialization simplifies.
\end{remark}
...
The self-trust mechanism $T_s$~\citep[Appendix~B]{lasser2026gfm} detects
...
The risk-trust $L^2$ convergence result of~\citet{lasser2026scm}
