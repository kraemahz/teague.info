...
is $\rS^{\mathrm{eff}} = c_{\mathrm{S}} \cdot \rhomincross +
c_{\mathrm{V}} \cdot \nu$, which is strictly larger than the exogenous
rate alone.  This widens the self-correcting basin of
Theorem~\ref{thm:phase_boundary}: the phase boundary condition becomes
easier to satisfy because the actor's endogenous incentive supplements
the observation-channel correction.

\begin{remark}[Stabilizing cascade]
\label{rem:stabilizing_cascade}
The endogenous correction term creates a stabilizing cascade that
mirrors the destabilizing cascade of the absorbing basin.  As the world
model improves ($\Lyap$ decreases), the actor makes better decisions,
which preserve more cooperative capabilities, which preserve more
channels, which provide more correction signal.  The destabilizing
cascade runs in reverse: channel loss degrades the model, which
produces worse decisions, which lose more channels.  The phase boundary
separates these two regimes, and the endogenous $c_{\mathrm{V}} \cdot
\nu$ term tips the balance toward the stabilizing side.  The two
cascades are asymmetric: degradation happens in discrete jumps
(subsumption removes an agent and its channels at a single step),
while recovery is continuous (calibration improves gradually through
