...
$\ln a_t$ is strictly negative and each $b_t$ is bounded per step),
the geometric mean of per-step multiplicative factors is $< 1$, and
the weighted suffix sums $\sum_s b_s \prod_{t>s} a_t$ are controlled
by the per-step bound on $b_t$ combined with the exponential decay
of the product tail.
