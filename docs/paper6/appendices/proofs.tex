...
correction signal is at least proportional to $\rhomincross(P_t) \cdot
\Lyap(\Wm_t)$ (the product of minimum cross-substrate redundancy and
total Lyapunov error).  The per-dimension argument: each channel
covering dimension~$k$ corrects at a rate proportional to
$\rho_k \cdot \epsilon_k^2$; summing across dimensions and bounding
below by $\rhomincross \cdot \Lyap$ yields C1.  This follows from the
...
$1 - \bar{a} = \rS \cdot \rhomincross - \rW \cdot \rsub$ (amortized
contraction rate) yields the explicit radius.  This neighborhood shrinks as $\Delta\rho \to 0$
...
signal vanishes and exogenous drift is non-negative (the true state
may change while the estimate cannot track it), $\epsilon_{j'}(t+1)
\geq \epsilon_{j'}(t)$.
...
This is plausible for the subsumption-vs-preservation action gap
(increasing error on any safety-relevant dimension decreases
preservation value), but is not guaranteed by the lemma's stated
assumptions; it is an additional structural condition on the action
gap function that holds when miscalibration uniformly
favors subsumption.  When this multi-dimensional monotonicity
condition holds, at
...
$\epsilon_{\mathrm{crit}}$ is therefore a conservative upper bound
