...
\end{equation}
The log-average form is the natural object because the first term of
\eqref{eq:iterated_bound} is a product; equivalently, the geometric
mean of $a_t$ is at most $\bar{a}$.  We require $a_t > 0$ throughout,
which holds whenever $\rW(t) \cdot \nsub(t) < 1$ (a step cannot more
than wipe out the current error); this is a weak condition on
per-step degradation and holds under C2$'$ for any finite learning
rate.
...
\end{equation*}
which is finite whenever the burst length of $a_t > \bar{a}$ is
uniformly bounded (a consequence of $\rsub$ being bounded below~1 and
$\rW$ being bounded above; see~\citet[Ch.~2]{liberzon2003switching}
for the switched-systems analogue).  Summing the geometric series:
\begin{equation}
...
Remark~\ref{rem:linearized}), $\ln a_t \approx a_t - 1$, so
\eqref{eq:uniform_contraction} is equivalent to
$\frac{1}{T}\sum (1 - a_t) \geq 1 - \bar{a}$.  Substituting the
worst-case values $\alphamin = \inf_t \alpha(t)$,
$\underline{\nu} = \inf_t \nu(P_t, \Wm_t)$, and the time-averaged
$\overline{\Delta\rho}$ yields
$1 - \bar{a} = \rS \cdot \rhomincross + \alphamin \cdot
c_{\mathrm{V}} \cdot \underline{\nu} - \rW \cdot \rsub$ and
$\bar{b} = \rsub \cdot \dmax \cdot \overline{\Delta\rho}$.  With
$\mu = 1$ in this regime (no overshoot when steps are small),
\eqref{eq:steady_state_bound} reduces to
Equation~\ref{eq:neighborhood}.
...
$\mathcal{N}$ is a neighborhood containing the trajectory.  The bound
is conservative (it assumes the action gap can respond to full
multi-coordinate error simultaneously) but finite whenever the strict
gap holds.  A cheaper per-coordinate estimator
$\min_k w_k \cdot \epsilon_{\mathrm{crit},k}^2$ rules out
single-coordinate flips but does not bound multi-coordinate flips,
so~\eqref{eq:eps_safe_bound} is the correct object for policy
correctness on the whole neighborhood.
