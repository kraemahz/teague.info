...
$< 1$ (which follows from $B < 0$ in the linearized regime; see the
inline proof of Theorem~\ref{thm:phase_boundary} for the precise
connection via $\ln a_t \approx a_t - 1$).  The second term converges
to a bounded steady-state value:
\begin{equation*}
...
$1 - \bar{a} = \rS \cdot \rhomincross + \alphamin \cdot
c_{\mathrm{V}} \cdot
\underline{\nu} - \rW \cdot \rsub$ (amortized contraction rate, with
$\alphamin = \inf_t \alpha(t)$ and
