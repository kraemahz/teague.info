...
\item[S0 (Valuation-bounded-by-Exercise).] For each agent~$i_n$
  ...
  estimation) but a per-event one-sided conservatism claim:
  for each individual event~$n$, the agent's valuation does not
  exceed the bundle's $W$-window exercise contribution.  The
  prose elsewhere uses ``systematically'' to describe the
  empirical rationale for adopting S0 (typical agents in
  typical markets do not pay more than what the capability
  delivers); this rationale is not part of the formal
  per-event statement, which holds or fails on each individual
  event.  The window-active
