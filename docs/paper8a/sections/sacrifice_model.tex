...
\item[\textbf{(P3)}] \textbf{Discretization as a feature.}  Trade
  ...
  content under the framework's standing assumption that the
  capability poset $P$ and category alphabet $\mathcal{C}$ are
  finite at evaluation time, but ``finite'' is not bounded
  uniformly across deployments---larger $P$ or $\mathcal{C}$
  produces larger per-event payloads.)
