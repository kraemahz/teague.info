...
\noindent\emph{Notation used throughout this section.}  A sacrifice
event is denoted as a tuple $(i_n, X_n, Y_n, t_n)$ with $i_n$ the
agent, $X_n$ the surrendered benchmarked capability (or bundle),
$Y_n$ the acquired unbenchmarked bundle, and $t_n$ the event
timestamp; the formal definition with its genuine-exchange
conditions appears as Definition~\ref{def:sacrifice_event} in
Section~\ref{sec:sacrifice_events}.  We use the tuple notation
in the assumption statements below.
...
\item[S0 (Valuation-bounded-by-Exercise).] For each agent~$i_n$
  ...
  \emph{window-active} realized $\volR$ contribution:
  ...
  $W$).  S0 is a per-event one-sided conservatism claim: for
  each individual event~$n$, the agent's valuation does not
  exceed the bundle's $W$-window exercise contribution.  The
  empirical rationale is that typical agents in typical markets
  do not pay more than what the capability delivers; this
  rationale supports adopting S0 but is not itself part of the
  per-event statement.  The window-active form is essential:
  the sacrifice channel observes acquisition at $t_n$, not
  exercise at any particular time, so S0's consequent is about
  the cumulative exercise witness across the window rather than
  the instantaneous value at $t_n$.  Agents who buy capabilities
  they never use within $W$ violate S0 for those events; the
  lower bound is about \emph{realized} rather than \emph{stored}
  capability.

\item[S1 (Free choice).] Each sacrifice event is a voluntary
  trade: agent~$i_n$ chose $Y_n$ over $X_n$ when both were
  available, the trade was free of \emph{material confounds}
  (no transaction costs that would distort the comparison
  beyond $\Delta\volL(X_n)$, no hidden side payments altering
  the effective $X$ or $Y$, no risk/expectation mismatch
  between $\Uin{n}(Y_n)$ at trade time and the realized
  $\volR^{[W]}(Y_n)$, and no unmodeled bundle attributes
  outside the declared $Y_n$ that contribute to valuation),
  and the agent had at least one \emph{non-degrading}
  alternative in their choice set: an option (retaining $X_n$,
  or some third option not equal to the trade itself) that
  would not have left the agent \emph{below-sufficiency} on
  any need dimension.  For this paper we treat
  ``below-sufficiency'' as a primitive: an event is
  below-sufficiency iff retaining $X_n$ would leave
  the agent unable to function on at least one need dimension
  without corrective intervention (food, water, shelter,
  medical care, etc.).  S1 fails for events in which
  \emph{no} non-degrading alternative exists in the agent's
  choice set (in particular: retention is degrading
  \emph{and} no third non-degrading option is available) or in
  which the material-confound conditions are violated; the
  institutional attestation layer
  (Remark~\ref{rem:external_attestation}) is responsible for
  filtering such events from the aggregate.  S1 holds (and the
  event is admissible) when retention is non-degrading, or when
  retention is degrading but the agent has a non-trade
  non-degrading alternative; the latter case requires
  attestation that the third alternative was actually available
  and non-degrading.  The full
  ...
  sufficiency, polarity-correct benchmarks, and the
  downstream-gated weighting that makes need-satisfaction a
  precondition for $\volL$-registration of downstream capability
  weight) is developed in the companion
  paper~\citep[Need~Sufficiency]{lasser2026paper8b}.
...
\item[S5 (Decomposition validity).] For events with
  ...
  share: $\alpha_{n,c} \leq \volR(c) / \volR(Y_n)$ for all
  such $n$ and $c \in Y_n$.  Events with $\volR(Y_n) = 0$
  carry no share information and are excluded from Part~B by
  the positivity precondition of the proof.  Under the Part~B
  hypothesis of within-bundle poset-independence
  (Definition~\ref{def:bundle_decomp}), $\sum_{c \in Y_n}
  \volR(c) / \volR(Y_n) = 1$, so the share lower bounds and
  the partition of unity $\sum_c \alpha_{n,c} = 1$ jointly
  force $\alpha_{n,c} = \volR(c)/\volR(Y_n)$ exactly: under
  Part~B's hypotheses, S5 collapses to a share-equality
  assumption rather than a strict inequality.  We retain the
  inequality form because it is the operationally natural
  default---practitioners typically have lower-bound share
  estimates rather than exact share knowledge---but
  acknowledge that under within-bundle independence the
  inequality is binding throughout, so Part~B is effectively
  conditional on the analyst supplying \emph{exact} share
  estimates compatible with the true (unknown) shares.
\end{description}
...
violating Definition~\ref{def:bundle_decomp}.  Combined with
the share-collapse note above, this means Part~B's strict
admissibility requires the analyst to supply
$\alpha_{n,c}$ values \emph{equal to the true (unknown)
$\volR$ shares}, not merely lower bounds.  In practice the
true shares are not known precisely; Part~B is therefore
operationally available only when the analyst can establish
either roughly-homogeneous bundle structure (where defaults
are close to true shares) or an externally-attested share
estimate.  S5 is a substantive empirical assumption, not
automatically satisfied by any single default.  The equal-weight default
$\alpha_{n,c} = 1/|Y_n|$ is \emph{not} a generic conservative
choice: it satisfies S5 only when the true $\volR$ shares are
each at least $1/|Y_n|$, which fails whenever any capability in
the bundle dominates the others.  Operational settings where S5
plausibly holds include: (a)~bundles known to be roughly
homogeneous in $\volR$ contribution (the equal-weight default
is then close to the true shares), (b)~explicitly conservative
attributions where $\alpha_{n,c}$ is set strictly below any
plausible share estimate from prior hedonic regressions
(\citealp{rosen1974hedonic}) or domain analysis, and (c)~bundle
configurations where one capability is known a~priori to
dominate and is given an attribution at most that capability's
true share.  Each of these is a context-specific assumption,
not a universal property of the decomposition.
\end{remark}
...
preference inequality (Step~2 of the justification in
Section~\ref{sec:sacrifice_events}).
...
paper~\citep[Need~Sufficiency]{lasser2026paper8b} characterizes when S1 fails
...
\emph{Systematic overpayment patterns.}  In markets with status
goods, speculative assets, or anticipation-driven purchases,
agents routinely sacrifice substantially more than the
exercise value they will realize within any reasonable window.
Ritualized payment (sacrifices made for participation rather
than benefit), gift aversion (sacrifices that signal commitment
beyond exercise contribution), and time-for-money inefficiency
(sacrifice-rate calibration that diverges from exercise
contribution because of preference-misalignment with the wage
channel) are all empirically prevalent failure modes of S0.
The theorem's empirical validity is bounded by the
prevalence of such violations in the trade pool being aggregated:
the bound is only as good as S0 holds across the events being
summed.

\emph{S0 is not diagnosable from trade data alone.}  Of the
theorem's load-bearing assumptions, S2 is checkable via the
benchmark function, and the genuine-exchange conditions (i)--(iv)
of Definition~\ref{def:sacrifice_event} are checkable
via trade records.  S0 is the
one assumption that cannot in principle be verified from the
trade pool because it concerns the relationship between
subjective valuation and realized exercise, neither of which
is directly observable in the sacrifice channel.  No
structural mechanism in this paper or its
companion~\citep[Need~Sufficiency]{lasser2026paper8b} diagnoses S0
failure; calibration of S0 is the work of
\emph{institutional attestation}
(Remark~\ref{rem:external_attestation}) acting on data sources
outside the sacrifice channel itself (e.g., longitudinal
exercise studies, market-segment behavioural analysis,
controlled-relaxation testing of the calibration chain). Open
Question~\ref{oq:calibration} is the formal extension; the
present limitation is that the bound's quality is bounded by
the institutional-attestation layer's quality on S0.
\end{remark}
...
$\Delta\volL(r_i \cdot h)$.  Currency and wage-time are inputs
to the benchmark function whose poset-measure-valued output is
what enters the calibration chain.
\end{remark}
...
sequence~\citep[SCM]{lasser2026scm}: it defines the timescale
over which the framework accumulates sacrifice evidence.
\end{definition}

\subsection{Sacrifice events and the cancellation lemma}
\label{sec:sacrifice_events}
...
(benchmark grounding), and S4(a) (valuation additivity) of
Section~\ref{sec:assumptions}.  S4(a) is needed for the
exchange-to-bundle cancellation
...
\begin{remark}[Cooperative closure of capability sets]
\label{rem:cooperative_closure_op}
Throughout this section, ``$\Ui(S)$'' for a finite capability
set $S$ is computed via S4(a) over the
\emph{cooperative-closure} of $S$: the set $S^{*}$ obtained by
adding to $S$ every cooperative capability $z \in P$ all of
whose participants are already in $S$.  Concretely,
$\Ui(S) = \sum_{c \in S^{*}} u_i(c)$, where the sum runs over
the closed set rather than the bare $S$.  Cooperatives whose
participants are not all in $S$ are not added, even if some are.
The genuine-exchange conditions (iii) and (iv) of
Definition~\ref{def:sacrifice_event} ensure cooperative
closures of $H \setminus X$, $X$, $Y$, $H$, and $(H \setminus
X) \cup Y$ behave compatibly: condition (iv) makes $H^{*} =
(H \setminus X)^{*} \sqcup X^{*}$ (no cooperative spans the
$X$/$H \setminus X$ split), and condition (iii) makes
$((H \setminus X) \cup Y)^{*} = (H \setminus X)^{*} \sqcup
Y^{*}$ (no cooperative spans the $Y$/$H \setminus X$ split).
The cancellation of Lemma~\ref{lem:cancellation} runs over
these closed sets.
\end{remark}

\begin{remark}[Bundle-for-bundle trades and mixed-polarity events]
\label{rem:bundle_for_bundle}
Definition~\ref{def:sacrifice_event} permits $X$ to be a
benchmarked capability \emph{or} bundle, and $Y$ is always an
unbenchmarked bundle, so bundle-for-bundle trades are
formally supported: the genuine-exchange conditions
(i)--(iv) and Lemma~\ref{lem:cancellation} operate on
set-level $X$ and $Y$ regardless of whether either side is
singleton.  When $X$ is a bundle, $\Delta\volL(X)$ is the
benchmark function evaluated on the bundle as a single
entity (typically via a transaction price or composite
benchmark covering the bundle), inheriting whatever
internal cooperative structure $X$ carries by the
cooperative-closure conditions.

A complication this paper does not formally resolve is the
\emph{mixed-polarity} bundle-for-bundle case in which the
same trade carries need-side and want-side polarities on
different components of the surrendered bundle.  A canonical
example: an agent who moves from one home to another
surrenders a bundle of capabilities (location, neighborhood,
school district, commute, etc.) and acquires a different
bundle of the same capability dimensions.  Some dimensions are
need-side (retention without shelter is degrading on the
shelter dimension), others are want-side (the new home's
kitchen is preferred but retention was non-degrading on that
dimension).  Under the current architecture, S1 admits such a
trade whenever \emph{any} non-degrading alternative existed
(retention via refinancing, renting elsewhere, etc.), and the
full $\Delta\volL(X)$ enters $\volRlower$ as want-side
preference revelation.  This is conservative for $\volRlower$
(it cannot inflate the lower bound) but loose for the
companion paper's need-access cost
diagnostic~\citep[Need~Sufficiency]{lasser2026paper8b}, which
counts the entire $\Delta\volL(X)$ of a mixed event as
need-access cost and so overstates true need-access cost when
most of the bundle is want-side.

The natural follow-up is an event-local bundle decomposition
that splits $\Delta\volL(X)$ across the surrendered bundle's
component capabilities with polarity tags, yielding a
need-share and a want-share separately.  This extension is
flagged as an open direction
(Section~\ref{sec:discussion}) and is the operational form
of the side-move-versus-upgrade disambiguation problem (a
trade that is a side-move on most dimensions and an upgrade
on one is currently indistinguishable on the ledger from a
trade that is an upgrade on most dimensions and a side-move
on one).
\end{remark}
...
\item \textbf{Calibration chain.}
  ...
  \item Acquisition-side calibration (S0):
    ...
    their \emph{window-active} exercise contribution.  The
    window-active form is what the observation channel
...
\emph{The cooperative-absorption argument is framework-internal,
not a general claim about real-world valuation functions.}  S4(a)
is plausible in this paper because the GFM framework's poset
construction is disciplined to internalize complementarities as
cooperative capabilities (M6); it is not the general claim that
any agent's valuation decomposes additively over bundle elements.
Outside the framework, S4(a) would be a strong empirical
hypothesis about real consumers; inside the framework, its
plausibility is inherited from the construction discipline that
puts each empirically-observed complementarity into the poset
explicitly as a cooperative node.  A poset that fails to do this
(complementarities present in the agent's valuation but absent
from the poset's cooperative structure) violates S4(a) by
construction; the assumption is therefore as strong as the
poset-construction discipline allows it to be, and no stronger.
This is the architectural reason S4(a) appears as an explicit
named assumption rather than a derived consequence, and the
reason its operational adequacy is a property of the
poset-construction governance layer, not of agent behaviour
per se.
\end{remark}
...
\item[\textbf{(P3)}] \textbf{Discretization as a feature.}  Trade
  ...
  per-event payload tuple $(i, X, Y, t)$ has finite information
  content under three standing assumptions: the capability
  poset $P$ and category alphabet $\mathcal{C}$ are finite at
  evaluation time, the agent universe is finite, and timestamps
  are recorded at a public bounded precision (i.e., $t$ takes
  values on a finite discretization grid spanning the trade
  window).  Under all three, ``finite'' is bounded by the
  per-event payload size; absent any one---e.g., real-valued
  timestamps with unbounded precision---the per-event payload
  is not bounded uniformly.)
...
\item[\textbf{(P4)}] \textbf{Commitment-layer composability.}
  ...
  $\Delta\volL$-magnitude, with counterparty and the specific
  benchmarked/unbenchmarked goods hidden by the commitment's
  hiding property (Section~\ref{sec:commitment}).  Note that
  $\Delta\volL(X)$ is a public function of the surrendered
  purchasing power on the money channel, so committing
  $\Delta\volL(X)$ is informative about the price~$p$ via
  benchmark inversion---the commitment hides
  \emph{counterparty and specific goods}, not price.
  Stronger price-hiding requires the hiding-variant rollup of
  Proposition~\ref{prop:zk_aggregation}, which releases only an
  aggregate threshold rather than per-event $\Delta\volL$.
\end{enumerate}
\end{theorem}
...
proof~\citep[Definition~7]{lasser2026exo} that $\Gamma$ binds
the event-local objectively verifiable facts: non-negativity
of $\Delta\volL(X_n)$, bundle containment $Y_n \subseteq
C_{\catYn{n}}$ (Definition~\ref{def:committed_event}(d)), and
a verifiable link to an underlying trade record.  The proof
does not certify ``valid revealed-sacrifice event'' in the
strict sense (all of S0, S1, S2, S3, S4(a), and the
genuine-exchange conditions, with S4(b) and S5 additionally
for Part~B); the non-objective hypotheses remain external
attestations (Remark~\ref{rem:zk_limits}).  This is the event-level
certification; S4(b) is not an event-level property but a
batch-level structural fact about the category partition,
established via pairwise-distinct category labels in
Proposition~\ref{prop:zk_aggregation} and inherited from the
poset-independence of distinct partition components
(Definition~\ref{def:category_partition}(i)).  S0, S1, S3,
S4(a), and S5 are \emph{not} ZK-verifiable under the default
commitment (Remark~\ref{rem:zk_limits}); S3 in particular
requires either external attestation that the bundle's
hedonic decomposition is non-degenerate, or a richer
commitment exposing decomposition fields.
...
\item[\textbf{(P2)}] \textbf{Consent via participation
  (institutional).}
  The aggregate lower bound sums event-magnitudes
  (Theorem~\ref{thm:aggregate_bound}, Part~A) or per-capability
  max-attribution quantities (Part~B) over observed events; in
  both Parts an agent with zero observed sacrifice events
  contributes zero to the aggregate.  This zero-contribution
  property is formal and holds under Part~A's lighter
  hypothesis stack as well as Part~B's; we state P2 in its
  Part-A form to keep the privacy claim independent of
  Part~B's stronger assumptions.  The \emph{operational}
  interpretation as ``consent'' additionally requires that
  participation in markets and labour exchanges be legally and
  socially opt-in; in settings where participation is coerced,
  the formal zero-contribution property holds but the consent
...
\item[\textbf{(P5)}] \textbf{Third-party observability
  ...
  $\Delta\volL(X(r_i \cdot h))$ is not third-party computable;
  ...
  relaxation~\citep[Controlled~Relaxation]{lasser2026wamura} (framework must impose a
  test) and with commitment-protocol
  verification~\citep[Exogenous~Verification]{lasser2026exo} (agent must generate a
