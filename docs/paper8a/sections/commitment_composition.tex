...
labels $\{\catY[_n]\}$ are \emph{pairwise distinct}, the
...
$\catY[_{n}] = \catY[_{n'}]$, both bundles lie inside the same
...
only trades satisfying S1's non-degradation (events that are
not below-sufficiency, per the primitive in
Section~\ref{sec:assumptions}) are admitted as sacrifice events;
(b)~the category partition $\mathcal{C}$ is curated so
cooperative-closure conditions (iii)--(iv) hold structurally;
(c)~S0 and S4(a) are assumed at the batch level with periodic
...
paper~\citep{lasser2026paper8b}; the present paper's theorems
hold under any institutional realization of the primitive.  The
...
\begin{enumerate}
\item[(a)] \emph{Sub-batch partitioning.} Partition the batch
  into maximal sub-batches whose labels are pairwise distinct
  and compute Part~A separately per sub-batch.  The resulting
  lower bounds are valid for each sub-batch's covered subset;
  the full-batch aggregate is the sum across sub-batches only
  when the sub-batches are themselves poset-independent, which
  is guaranteed when sub-batches are built from distinct
  partition components.  A single-component batch with multiple
  events yields at most the per-event Part~A bound,
  $\volR(Y_n) \geq \Delta\volL(X_n)$ for each $n$; aggregation
  requires the additional independence.
