...
attestation locus:
(a)~the trade environment is filtered so only trades satisfying
S1's non-degradation (events that are not below-sufficiency, per
the primitive in Section~\ref{sec:assumptions}) are admitted as
sacrifice events;
...
$(\Delta\volL(X_n), \catYn{n})$ in cleartext; the ZK proof
$\pi_n$ is strengthened to additionally attest that
$\catYn{n} \in \{1, \ldots, m\}$ (a valid partition label).
...
\item[(ii)] the \emph{non-null} bundle category labels
  $\{\catYn{n} : \catYn{n} \neq \mathrm{null}\}$ are
  pairwise distinct (i.e., among the $N \leq N_{\max}$ real
  events, no two share a category label; the $N_{\max} - N$
  null-padded positions are excluded from the
  pairwise-distinct check, so multiple null entries do not
...
with their openings $(\Delta\volL(X_n), \catYn{n}, r_n)$.  To
...
$(\catYn{1}, \ldots, \catYn{N_{\max}})$, where positions
