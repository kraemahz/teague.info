...
paper~\citep[paper~8b]{lasser2026paper8b}; the present paper's theorems
...
\citep[paper~8b]{lasser2026paper8b}: every assumption that the
...
order~$p$.  Floor rounding makes the encoded sum a lower
bound on $\sigma$ times the true real-valued sum:
$\mathrm{encoded\_sum} \leq \sigma \cdot
\mathrm{true\_sum}$, but
$\mathrm{encoded\_sum} \geq \sigma \cdot \mathrm{true\_sum} -
N$ where $N$ is the actual batch size (each event loses at
most $1$ in the encoding).  Equivalently,
$\mathrm{true\_sum} \geq (\mathrm{encoded\_sum} + N) / \sigma$.
A range-proof on the encoded sum certifying
``$\mathrm{encoded\_sum} \geq \sigma B - N_{\max}$''
therefore implies $\mathrm{true\_sum} \geq B$ for any actual
$N \leq N_{\max}$.  The threshold passed to the range-proof
must be \emph{below} the desired real-valued threshold by the
worst-case rounding margin $N_{\max}/\sigma$ (in true-sum
units), not above it.
