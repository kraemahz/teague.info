...
order~$p$.  Floor rounding ensures the encoded sum is a lower
bound on $\sigma$ times the true real-valued sum, so a
range-proof certifying ``encoded sum $\geq \sigma B$'' implies
``true sum $\geq B$'' directly.  Rounding error per event is
bounded by $1/\sigma$ in $\volL$-units and propagates
additively across the batch ($N \leq N_{\max}$ events giving
total rounding bound $N/\sigma$); the threshold~$B$ should be
set with margin $\geq N_{\max}/\sigma$ above the true bound
to absorb worst-case accumulation.
