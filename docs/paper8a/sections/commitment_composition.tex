...
\item $\pi$ is a zero-knowledge
  ...
  \item[(a)] the commitment $\Gamma$ corresponds to a valid
    ...
    verifiable components: the objective component of S2
    ($\Delta\volL(X)$ correctly computed from~$X$ via the
    public benchmark);  S3 (bundle coherence:
    non-degenerate hedonic decomposition) is \emph{not}
    certified by this proof---it requires either
    institutional attestation (Remark~\ref{rem:external_attestation})
    or a richer commitment with explicit decomposition fields
    (cf.\ Remark~\ref{rem:commit_part_b} for the Part~B
    extension);
...
benchmark computation, and S4(b) via poset structure on the
public partition); the remaining conditions are structural
...
choice), S3 (bundle coherence), S4(a) (valuation additivity),
and the cooperative-closure conditions (iii)--(iv)---are about
agent-internal state, per-bundle hedonic structure, or
poset-structural facts the default commitment protocol cannot
itself attest (Remark~\ref{rem:zk_limits}).  S3 in particular
requires external attestation under the default commitment;
Part~B's per-capability use of S3 inherits this dependency.  Their validity must be
...
\Delta_{\max} \rfloor]$, and the aggregate fits in
$[0, N_{\max} \cdot \lfloor \sigma \cdot \Delta_{\max}
\rfloor] \subset \mathbb{Z}_p$ for the curve order~$p$.
Rounding error per event is bounded by $1/\sigma$ in
$\volL$-units and propagates additively across the batch
($N \leq N_{\max}$ events giving total rounding bound
$N/\sigma$); the threshold~$B$ is set with margin
$\geq N_{\max}/\sigma$ to absorb worst-case accumulation.
