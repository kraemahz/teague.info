...
\[
  ...
  time } t\},
\]
where ``exercised at $t$'' is formalized via the
counterfactual exercise condition of
Appendix~\ref{app:exercise_indicator}
(Definition~\ref{def:exercise_indicator}): $d$ is exercised at
$t$ iff some realized cooperative output in the lookback
window $[t - \Delta, t]$ becomes unrealizable when $d$ is
removed from $O$.  The lookback width $\Delta$ and the
exercise indicator's other parameters are specified in
