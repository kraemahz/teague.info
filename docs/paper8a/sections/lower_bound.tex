...
Volume]
\label{def:volR}
Let $P$ be the full capability poset with the poset measure
$\volL$~\citep[Definition~7]{lasser2026poset} satisfying axioms
M1--M6~\citep[Proposition~1]{lasser2026poset}.
...
\[
  ...
  time } t\},
\]
where ``exercised at $t$'' is formalized
\emph{semantically} via the counterfactual exercise condition:
$d$ is exercised at $t$ iff some realized cooperative output in
the lookback window $[t - \Delta, t]$ becomes unrealizable when
$d$ is removed from $P$.  This is the latent structural notion;
it does not depend on any perimeter or measurement mechanism.
Appendix~\ref{app:exercise_indicator}
(Definition~\ref{def:exercise_indicator}) gives the
\emph{operational} realization of this counterfactual within an
internal observation perimeter $O \subseteq P$, where the
indicator is computable; outside~$O$ the counterfactual remains
the latent definition of $P^{\mathrm{ex}}_t$ but is not
directly measured by the indicator.  The lookback width
$\Delta$ and the indicator's other parameters are specified in
Definition~\ref{def:exercise_indicator}; ``at $t$'' here
denotes that $t$ is the right endpoint of the lookback,
not that the exercise happens instantaneously at $t$.  The \emph{instantaneous
...
\medskip\noindent
\textbf{Window-active form (union over time).}
...
Each bundle $Y_n$ is taken as \emph{cooperative-closed}: $Y_n$
contains a cooperative capability $z$ iff all participants of
$z$ are in $Y_n$.  For each event~$n$:
\[
...
$n \neq n'$).  We additionally require set-disjointness
$Y_n \cap Y_{n'} = \emptyset$ for $n \neq n'$, which is
implicit when subsumption is taken as a strict partial order
on distinct capabilities; we make it explicit here so that
the proof's M4 step does not have to invoke reflexivity of the
relation:
\[
  \volR^{U,[W]} \;\geq\; \sum_{n=1}^{N} \Delta\volL(X_n),
\]
where $U = \bigcup_n Y_n$ and $\volR^{U,[W]}$ is the
window-active realized capability volume restricted to $U$.
Pairwise poset-independence makes $U$ itself
cooperative-closed (no cooperative spans any pair $Y_n, Y_{n'}$,
so each cooperative whose participants lie in $U$ has
participants entirely in some single $Y_n$).  Part~A is the paper's
...
and each is cooperative-closed (no cooperative is split between
$Y_n$ and any other bundle, and each $Y_n$ contains its own
internal cooperatives).  Then the sub-posets carried by the
bundles are disconnected components of the sub-poset on
$U = \bigcup_n Y_n$, and $U$ inherits cooperative closure from
the per-bundle closure plus pairwise independence.
Axiom~M4 of~\citep[Proposition~1]{lasser2026poset} makes the
poset measure additive over poset-disjoint cooperative-closed
components; $\volR^{[W]}$ inherits M4
...
and wireheading diagnostic~\citep[Need~Sufficiency]{lasser2026paper8b} are stated
...
the maximum attribution across all observed events, with the
zero-extension convention $\volRlower(c) = 0$ when $c$ is in
no observed bundle:
\[
  \volRlower(c) \;=\;
  \max_{n:\, c \in Y_n} \alpha_{n,c} \cdot \Delta\volL(X_n),
  \qquad
  \text{or } 0 \text{ if no event covers } c.
\]
Aggregating over a fixed common universe $U^* \supseteq
\bigcup_{n \in E'} Y_n$ (any superset of the largest observed
$U$ across the event sets compared, with capabilities outside
the observed bundles contributing zero by the convention
above), $\volRlower(E) := \sum_{c \in U^*} \volRlower(c \mid E)$
satisfies: for event sets $E \subseteq E'$ both satisfying
...
\[
  ...
  \sup_{B \in \mathcal{B}(E)}\, \sum_{n \in B}
  \Delta\volL(X_n),
\]
where $\mathcal{B}(E)$ is the family of sub-batches $B
\subseteq E$ satisfying the Part~A admissibility and pairwise
poset-independence hypotheses of
Theorem~\ref{thm:aggregate_bound}, with the supremum attaining
$0$ on the empty sub-batch.  Then
