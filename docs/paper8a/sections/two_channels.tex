...
identification).  \emph{Admissibility cap.}  Through the
calibration chain
$\Ui(Y) \geq \Ui(X) \geq \Delta\volL(X)$ (revealed preference
plus S2) combined with S0
($\Ui(Y) \leq \volR^{[W]}(Y) \leq \sum_{c \in Y} w(c)$), the
event-magnitude $\Delta\volL(X(r_i \cdot h))$ is implicitly
capped at the bundle's window-active $\volR$ ceiling; events
where the wage-imputed magnitude exceeds this ceiling violate
the assumption stack and are inadmissible
(Remark~\ref{rem:s0_time_cap} below).
\end{definition}
...
Examples~\ref{prop:agent_specific_time}
...
residuals: Example~\ref{prop:agent_specific_time} witnesses
a capability captured by time but not by money;
Example~\ref{prop:capital_concentrated} witnesses a
