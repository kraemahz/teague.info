...
\item \textbf{Third-party observability.}  Money-sacrifice events
  ...
  labels); auditors can compute the
  \emph{public-ledger flow aggregate} without agent cooperation
  (Property~P5 of
  Proposition~\ref{prop:privacy_institutional}).  Computing
  the \emph{theorem-admissible} aggregate
  $\sum_n \Delta\volL(X_n)$ that
  Theorem~\ref{thm:aggregate_bound} certifies as a $\volR$
  lower bound additionally requires the public
  S1-admissibility labelling and the other
  external-attestation outputs of
  Remark~\ref{rem:external_attestation} (genuine-exchange
  conditions, S0 calibration, S4(a)).  Ledger auditability is
  the data-availability property; admissibility-label
  auditability is a separate institutional property whose
  realization is governance-supplied.
\end{itemize}
...
\item \textbf{Public versus private sacrifice.}  Publicly
  ...
  at the data-availability level; computing the
  theorem-admissible time-channel aggregate also requires the
  public S1-admissibility labelling and the other
  external-attestation outputs of
  Remark~\ref{rem:external_attestation}, mirroring the
  money-channel split above.  Private time-sacrifice (parenting,
  contemplation) is visible
...
(characterized in the companion paper~\citep[Need~Sufficiency]{lasser2026paper8b}):
