...
(Theorem~\ref{thm:aggregate_bound}) lower-bounds the
sacrifice-channel-observable fraction $\betaL$ (equivalently,
upper-bounds $1 - \betaL$) on the observed subset, under
S0--S2, S4(a), and the genuine-exchange conditions of
Definition~\ref{def:sacrifice_event}.  Because
$\betaL \leq \beta = \volR/\volL$ by construction, this also
lower-bounds the true exercised fraction $\beta$ on the observed
subset; the constructed object is $\betaL$ and the true $\beta$
is bounded only as a consequence, not measured directly.  The
