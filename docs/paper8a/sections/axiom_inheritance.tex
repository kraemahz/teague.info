...
\item[(M6) Superadditivity under independence.]
  ...
  \item[(b)] \textbf{$\volRlower$: event-set non-additivity
    (a property of the lower-bound construction, not an M6
    statement).}
    M6 is a measure axiom on poset-independent capability
    subsets, and applies to $\volR$ on a fixed exercised
    sub-poset by the restriction argument of (a).  $\volRlower$
    is \emph{not} a measure of $\volR$'s restriction; it is an
    observation-derived lower-bound functional of trade events.
    M6 is therefore not a property we have to inherit for
    $\volRlower$, and we make no claim that $\volRlower$ is
    super- or sub-additive on capability subsets.
    A separate property holds at the \emph{event-set}
    level: merging two groups of sacrifice events changes
    per-capability attributions under max-attribution, and the
    joint maximum over all events can be less than the sum of
    within-group maxima.  This is a non-superadditivity property
    of the lower-bound construction over event-set unions
    (Example~\ref{ex:m6_fail_volRlower} illustrates it in a
    constructed scenario outside Part~B's strict admissibility
    regime), retained for operational clarity about what
    max-attribution does on merged event groups.  M4 still
    applies to $\volRlower$ on poset-independent capability
    subsets through the per-capability max-attribution sum:
    $\volRlower(A \cup B) = \volRlower(A) + \volRlower(B)$ when
    $A, B$ are poset-disjoint, since the per-capability sums
    decouple over disjoint cooperative-closed subsets.
    M5 (every valid capability has positive weight) applies to
    $\volR$ on the exercised sub-poset by restriction, but
    $\volRlower$ assigns zero to any capability not covered by
    an admissible event, so $\volRlower$ is not an M5-style
    measure on the unexercised side.
  \end{enumerate}
\end{description}
\end{theorem}
...
The non-invariance lives at the level of the
...
holds; strict monotonicity fails, and the mapping is not
strictly contracted by capability removal when the removed
capability is unexercised.  M6 itself remains satisfied on the
fixed exercised sub-poset $P^{\mathrm{ex}}_W$ by restriction;
the mapping-level non-invariance is what breaks the
self-balancing transfer
...
within-group aggregates.  Max-attribution operates over events
covering each capability rather than over poset-independent
capability subsets, so the failure is on event-set merging
specifically.
...
$\volL$ to $\volR$.  The GFM self-balancing
...
\begin{remark}[When the operational analogues of M1--M6 hold
for $\volRlower$]
The event-set non-additivity of (b) above does not arise when
the merged groups have sacrifice events in poset-independent
bundle categories (Definition~\ref{def:category_partition}):
each unbenchmarked capability appears in sacrifice events from
only one group, and cross-group capabilities are
poset-independent.  Under this non-redundancy condition,
$\volRlower$ satisfies the operational analogues of M1, M2,
M3, M4, and (the M6-analogue stated in (b)) consistently;
$\volRlower$ remains a lower-bound functional, not a
poset measure, so this is operational compatibility rather
than literal axiom inheritance.  Even so, the self-balancing
property transfers to $\volRlower$ only as a \emph{lower
bound} on self-balancing---$\volRlower$ being self-balancing
does not imply $\volR$ itself is self-balancing.
\end{remark}
