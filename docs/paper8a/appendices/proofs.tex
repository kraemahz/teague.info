...
\begin{enumerate}
\item Revealed-preference observation: $\Uin{n}((H_n \setminus
  X_n) \cup Y_n) \geq \Uin{n}(H_n)$.
\item Cancellation to bundle comparison (S4(a) +
  ...
  $\Uin{n}(Y_n) \geq \Uin{n}(X_n)$.
\item Calibration chain (S0 + S2), S0 stated directly in
  window-active form:
  $\volR^{[W]}(Y_n) \geq \Uin{n}(Y_n) \geq \Uin{n}(X_n)
...
\item \emph{Decomposition validity (S5, Part~B):} hedonic
  ...
  bridge.
\item \emph{Calibration (S0):} the substantive bridge of
  the chain (Remark~\ref{rem:s0_substantive})---a one-sided
  conservatism claim whose empirical verification is the
  hardest among the assumption stack (Open
