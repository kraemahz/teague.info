...
Capability descriptions are the atomic currency of the poset, analogous to
...
connects to the empowerment interpretation of the foundational paper
\citep{klyubin2005empowerment}: the information content of a benchmark is
analogous to the channel capacity of the capability's action-state mapping.
...
\begin{definition}[Capability Poset]
\label{def:capability_poset}
The \emph{capability poset} $(\C, \leq)$ is the finite partially ordered
...
inclusion-exclusion correctly even when the poset has branching and shared
...
\begin{definition}[Poset Measure $\volL$]
\label{def:poset_measure}
For a set of capabilities $G \subseteq \C$, define the \emph{poset measure}:
\begin{equation}
\label{eq:poset_measure}
...
includes everything it subsumes. The poset measure sums the independent
...
If $(\C, \leq)$ is a \emph{forest} (every element has at most one cover),
    ...
    \lessdot d$. For general posets with branching, the sufficient condition
...
On a general poset, the M\"{o}bius function can produce alternating-sign
...
function and the poset topology, verified at poset construction time.
\end{proof}

\paragraph{Practical scope of the constraint.} Capability posets in practice
...
prerequisites (musicapp subsumes both webapp and perform). The worked example
in Section~\ref{sec:example} demonstrates a poset that passes the check for
all nodes including its diamond.

\paragraph{Non-negativity insertion policy.} When a proposed subsumption edge
set would make $\iw(d) < 0$, the actor follows a deterministic policy to
eliminate strategic latitude at the insertion point:

\begin{enumerate}
\item \textbf{Attempt benchmark refinement.} Query the communication and
  observation channels for a finer-grained benchmark for $d$. If $\smax$ can
  be increased enough that $\iw(d) \geq 0$, adopt the refined benchmark and
  add the edges.
\item \textbf{Defer the edges.} If no finer benchmark is available, add $d$ to
  the poset as an unstructured atom (no subsumption edges) and record the
  proposed edges in a deferred-edge queue. The capability contributes its
  binary default weight immediately.
\item \textbf{Re-check on benchmark updates.} Whenever any capability in the
  deferred-edge queue or its prerequisites receives a benchmark refinement, the
  actor re-evaluates the non-negativity condition for all queued edges
  involving those capabilities. Edges that now pass are committed.
\end{enumerate}

This policy is deterministic: the actor always prefers structure (step~1) over
isolation (step~2), and deferred edges are re-evaluated automatically (step~3).
The actor has no discretion over \emph{whether} to add edges, only over
\emph{when} sufficient measurement precision exists to support them. This
closes the strategic manipulation concern at the insertion point: the actor
cannot choose to decline edges for vol-strategic reasons because the policy
mandates accepting them as soon as the non-negativity check passes.

\paragraph{Cooperative capabilities.} A cooperative capability $d_{\mathrm{coop}}$
is a poset node whose benchmark conditions require the participation of
...
0 to 100. Cooperative capabilities enter the poset through the same channels
...
The poset is populated from two independent channels, providing robustness
...
claim enters the poset weighted by trust:
\begin{equation}
\label{eq:w_comm}
...
requires $d$). The observation enters the poset weighted by observational
confidence:
\begin{equation}
\label{eq:w_obs}
...
$\volL$: the poset structure is the same regardless of how nodes are
populated. It increases the \emph{quality} of the poset by providing
