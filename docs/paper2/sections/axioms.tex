...
\begin{proposition}[Lattice Measure Satisfies M1--M6]
\label{prop:axioms}
The lattice measure $\volL$ (Definition~\ref{def:lattice_measure}) satisfies
...
\item[\textbf{(M4)}] If $A$ and $B$ are \emph{lattice-disjoint}
    (Definition~\ref{def:lattice_disjoint}), then
...
\item[\textbf{(M6)}] When agents' individual capability sets occupy
    incomparable regions of the lattice, $\volL(G) \geq \sum_k
...
requires on the lattice.

\begin{definition}[Lattice-Disjoint]
\label{def:lattice_disjoint}
Two sets $A, B \subseteq \C$ are \emph{lattice-disjoint} if no element of $A$
...
lattice-disjoint (one subsumes the other). Lattice-disjointness ensures that
...
(Section~\ref{sec:lattice}), $\iw(d) \geq 0$ for all $d \in \C$. Since
...
\emph{(M4) Finite additivity under lattice-disjointness.} If $A$ and $B$ are
lattice-disjoint, then $\dc A \cap \dc B = \{\bot\}$. The downward closure of
...
\emph{(M5) Non-triviality.} For any atom $d$ of the lattice (a minimal
...
regions of the lattice (no agent's capability subsumes another agent's
capability), the sets are pairwise lattice-disjoint. By (M4) applied
...
\begin{theorem}[Self-Balancing on Finite Lattices]
\label{thm:self_balancing_lattice}
The self-balancing property (Proposition~1 of \citet{lasser2026gfm}) holds for
    $\volL$ on finite weighted lattices. Specifically: destructive
