...
\title{C7 closed-form derivation, v5: \\
Lemma 1 over $H_j$, normalized conditional baselines, \\
explicit $H_j$ membership in aggregate suprema}
...
Same as v4: derive (C7) bounded co-evolution as a corollary of
(C5.HOEFF), (C7.RATE), and channel-projection structure.

\subsection{Changes from v4 (codex review responses)}

\begin{enumerate}
\item \textbf{$H_j$ scoping carried through to Lemma 1 (P1).}  v4
  defined $M_{j \to j'}^{\mathrm{step}}$ over $H_j$ but Lemma~1's
  proof was stated for $\Delta \in \mathcal{D}_j$.  v5 proves
  Lemma~1 for $\Delta \in H_j$ directly (the inequality is purely
  algebraic; positivity of $q_0 + \Delta$ is not needed).  v5 also
  downgrades v4's equality-of-suprema claim to the upper-bound
  direction; equality requires $q_0$ to have strictly positive
  mass on every $a \in A_j$, which we do not assume.

\item \textbf{Conditional baseline normalization fixed (P1).}  v4
  defined $p_0^{(j)} := \bar{p}_0^{(j)}|_{V_j}$, which is a
  sub-probability measure of mass $1 - \epsilon_j^{(0)}$, but
  called it ``the conditional baseline'' as a probability measure.
  v5 normalizes: $p_0^{(j)}(v) := \bar{p}_0^{(j)}(v) / (1 -
  \epsilon_j^{(0)})$, defined when $\epsilon_j^{(0)} < 1$.
  Assumption~1's projective consistency is restated using the
  conditional pushforward
  $\mathrm{Law}(\pi_j(a) \mid a \in A_j) = p_0^{(j)}$.

\item \textbf{Aggregate suprema membership made explicit (P2).}
  v4's Lemma~2(b1)/(b2) suprema listed only the budget condition.
  v5 adds explicit $\Delta_n^{(j)} \in H_j$ membership to the
  displayed suprema.
\end{enumerate}

\section{(legacy) Goal and changes from v3}
...
\item \textbf{Projective consistency type-corrected (P1).} v3's
  ...
  \{*_j\}$, with the \emph{normalized} conditional baseline
  $p_0^{(j)}(v) := \bar{p}_0^{(j)}(v)/(1 - \epsilon_j^{(0)})$
  given engagement (v5 fix; v4 used the un-normalized restriction)
  and $\epsilon_j^{(0)} := \bar{p}_0^{(j)}(\{*_j\})$ as the
  baseline null-mass.
...
\item An \textbf{extended baseline distribution} $\bar{p}_0^{(j)}$
  ...
  including the engagement/null bucket; $\epsilon_j^{(0)} :=
  \bar{p}_0^{(j)}(\{*_j\}) \in [0, 1)$ is the baseline null-mass.
  The \textbf{conditional baseline given engagement} is the
  normalized restriction
  \[
    p_0^{(j)}(v) \;:=\; \frac{\bar{p}_0^{(j)}(v)}{1 -
    \epsilon_j^{(0)}}, \qquad v \in V_j,
  \]
  defined when $\epsilon_j^{(0)} < 1$; $p_0^{(j)}$ is then a
  probability measure on $V_j$.
...
\epsilon_j^{(0)}$, and the conditional pushforward (the normalized
law of $\pi_j(a)$ given $a \in A_j$) equals the conditional
baseline:
\[
  \mathrm{Law}\bigl(\pi_j(a) \,\big|\, a \in A_j\bigr) \;=\;
  p_0^{(j)} \quad\text{on } V_j.
\]
...
constraint $q_0 + \Delta \geq 0$).  The functional
$\Delta \mapsto \Delta\mu_{j'}[\Delta] = \sum_{a \in A_{j,j'}}
\Delta(a) \ell^{(j')}(a)$ extends linearly from $\mathcal{D}_j$ to
all of $H_j$.  We work with the operator norm of this functional
on $H_j$; the operator-norm bound on $H_j$ then upper-bounds the
admissible-restricted supremum
$\sup_{\Delta \in \mathcal{D}_j \setminus \{0\}}
|\Delta\mu_{j'}[\Delta]|/\|\Delta\|_1$ by the inclusion
$\mathcal{D}_j \subseteq H_j$.  Equality of these two suprema
holds when $q_0$ has strictly positive mass on every $a \in A_j$
(so $\mathcal{D}_j$ contains a neighborhood of $0$ in $H_j$); we
do not require this equality, only the upper-bound direction.
...
on $H_j$ (in the $\ell_1$-to-$\mathbb{R}$ sense).  By the
inclusion $\mathcal{D}_j \subseteq H_j$, the operator norm on
$H_j$ upper-bounds the admissible-restricted supremum
$\sup_{\Delta \in \mathcal{D}_j \setminus \{0\}}
|\Delta\mu_{j'}[\Delta]|/\|\Delta\|_1$.  Equality of the two
suprema requires $q_0$ to have strictly positive mass on every
$a \in A_j$ (so that $\mathcal{D}_j$ contains a neighborhood of
$0$ in $H_j$); we do not assume this --- only the upper-bound
direction is required for the deployment claim.
\end{remark}
...
\begin{proof}
For any $\Delta \in H_j$ with $\Delta \neq 0$ (the bound holds on
the full linear space, not just on the admissible subset
$\mathcal{D}_j$, since the inequality below uses only the
zero-outside-engagement convention and (C5.HOEFF)'s clip bound,
not positivity of $q_0 + \Delta$):
\begin{align*}
...
\setminus \{0\}$ gives \eqref{eq:per-step-bound-v3}.  When
...
\item \textbf{Does the projective consistency condition
  ...
  $(\pi_j)_* q_0 = \bar{p}_0^{(j)}$ on $V_j \cup \{*_j\}$ for
  every $j$ (equivalently, $\mathrm{Law}(\pi_j(a) \mid a \in A_j)
  = p_0^{(j)}$ on $V_j$), with the extended per-channel baselines
  being simultaneously realizable.  In practice this constrains
  how per-channel baselines are calibrated.  Is this a reasonable
