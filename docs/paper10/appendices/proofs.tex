...
correction terms in the structure of $X$ are bounded by
condition~(C7) (bounded co-evolution), now derived as a corollary
in \citep[Theorem~1]{lasser2026foundations} from (C5.HOEFF),
(C7.RATE), and the verification protocol's channel-projection
structure.
...
$(\epsnonresCoev)_+$ as open work; Paper~10 closes the gap via
condition~(C7) (bounded co-evolution), now derived as a
corollary in \citep[Theorem~1]{lasser2026foundations}.

\emph{Step 4 (bounded co-evolution).}  By condition~(C7) and
\citep[Theorem~1]{lasser2026foundations}, there exists
...
\emph{Step 6 (apply Lyapunov bound).}  Under invariants $I_1, I_3$,
...
SPRT detection time (in SPRT exposure-event count) for a violation
producing post-clipping LLR drift $\delta > 0$ under the
alternative distribution.  Then for $t > A/\delta$, where $A =
\log((1-\beta)/\alpha)$ is the SPRT threshold:
\[
  \Pr[T_{\mathrm{detect}} > t]
  \;\leq\; \exp\!\left(-\frac{2(t\delta - A)^2}{t \Rh^2}\right),
\]
with $\Rh = b - a = 2\Bclip$ the Hoeffding range width of the
clipped LLR (per (C5.HOEFF)).  Asymptotically, in the regime
$t\delta \gg A$:
\[
  \Pr[T_{\mathrm{detect}} > t]
  \;\leq\; \exp(-\kappa \cdot t \cdot \delta),
\]
with $\kappa = 2\delta/\Rh^2$.  The wall-clock-to-event-count
comparison (lead-time-before-cascade) is composed in
Lemma~\ref{lem:6} via (C11.CLK), \emph{not} in this lemma.
...
\emph{Mean bound (Wald's identity).}  By Wald's
identity~\citep{wald1947sequential} for the SPRT (\citep[Proposition:
Behavioral Monitoring Detection Rate]{lasser2026exo}):
\[
...
Set $R = b - a$. By Hoeffding's
inequality~\citep{hoeffding1963probability,azuma1967weighted}
(applied via the martingale-difference form for adapted increments):
\[
...
A \text{ for all } s \leq t\}$.  A \emph{necessary} condition for
this is $\Lambda_t < A$ (the LLR has not yet crossed the threshold
at time $t$); the joint event implies the marginal-time event.
Therefore:
\[
...
\]
The inequality goes the right way precisely because we relax to a
necessary condition: the joint-event probability is at most the
...
\emph{Asymptotic form.}  In the regime $t\delta \gg A$ (event-
count exposure long relative to mean detection threshold-crossing),
$t\delta - A \approx t\delta$, so:
\[
  \Pr_{p_1}[T_{\mathrm{detect}} > t]
  \;\leq\; \exp(-2 t \delta^2 / R^2)
  \;=:\; \exp(-\kappa \cdot t \cdot \delta),
\]
with $\kappa = 2\delta/R^2$ (a sub-Gaussian-like rate constant).
This is the form Lemma~\ref{lem:6} inverts (Substeps 5a--5e of
Appendix~\ref{app:proof_lem6}) to derive the high-probability
detection quantile $\Tbeta$ in SPRT exposure-event count.

\textbf{Note on cascade-clock composition.}  Lemma~\ref{lem:4}
itself is stated in the SPRT exposure-event clock $t$.  The
lead-time-before-cascade guarantee --- comparing $T_{\mathrm{detect}}$
against wall-clock cascade time --- is composed in
Lemma~\ref{lem:6} (Appendix~\ref{app:proof_lem6}) via (C11.CLK)'s
event-count floor $\Ncasc$, clock-failure probability $\bclk$, and
calibration-uncertainty $\bcal$.  This separates the SPRT-step
tail (an event-count statement) from the cascade-clock event (a
wall-clock-to-event-count comparison) cleanly.

\textbf{Caveat on
\citep[Phase~Redundancy]{lasser2026phase}'s scaling form.}
\citep[Phase~Redundancy]{lasser2026phase} states
$\tau_{\mathrm{meta}} \gtrsim C/I_k$ as a scaling law, not a
theorem-level bound.  Lemma~\ref{lem:6}'s composition handles
this via (C11.CLK)'s deterministic event-count floor and the
named failure probabilities, rather than treating
$\tau_{\mathrm{meta}}$ as a hard inequality.  Deployments where
the scaling does not hold tightly require larger $\bclk$ or
$\bcal$ to absorb the slack.
\end{proof}
...
For multinomial distributions over capability partitions, the KL
is
\[
  D_{\mathrm{KL}}(q \,\|\, p_0) \;=\; \sum_i q_i \log(q_i / p_{0,i}).
\]
For $\delta_3$ and $\delta_4$ to be strictly positive deployment-
class constants, we require:
\begin{itemize}
\item \textbf{Fixed finite categories.}  The multinomial partitions
  for Channels~3 and~4 are fixed before deployment with bounded
  cardinality $K_3, K_4 \leq \Kchmulti$, where
  $\Kchmulti$ is a deployment-class constant ((C5.MULT)
  applied to the agent-side multinomial channels).  Categories
  cannot be added during deployment.
\item \textbf{Baseline mass floor.}  Every category $i$ has
  $p_{0,i} \geq \eenv > 0$, with $\eenv$ a deployment-class
  regularization constant ((C5.SUPP) extended for agent-side
  multinomials, mirroring the environment-side support
  conventions).  Without the mass floor the least-favorable
  perturbation can drive $\delta_3, \delta_4 \to 0$ via a
  vanishing-baseline-mass cell.
\end{itemize}
Under these conditions, the constraint that at least one component
shifts by $\eta_3$ (or $\eta_4$) yields a least-favorable choice
that concentrates the perturbation on the component with smallest
baseline mass.  The KL floor $\delta_3$ (or $\delta_4$) is the
infimum over feasible $q$ subject to the shift constraint and the
support floor, and is strictly positive.
...
strictly different from the baseline under the stated support
conditions; the minimum is therefore strictly positive.
...
procedure.
\subsection{Proof of Lemma 6 (\texorpdfstring{$h_{\mathrm{detect}}$}{h\_detect} intensivity)}
...
per-SPRT-exposure-step gap growth satisfies
$(\Delta\epsgap)_n \leq \rhogap$ uniformly over the admissible
adversarial class, so
$\epsgap(s_n) \leq \epsgap(s_0) + n \cdot \rhogap$.
...
\[
  ...
  \;\leq\; h_{\mathrm{static}}(\theta) + \rhogap \cdot
  \Tbeta.
\]
...
$\rhogap$ intensive by (C11); $\Tbeta = \max\{2A/
...
$T_{\mathrm{detect}}$ from Lemma~4's Hoeffding form), measured in
SPRT exposure-event count.  Separately, $\tau_{\mathrm{meta}} \leq
T_{\mathrm{cascade}}$ is a \emph{lower} bound on wall-clock cascade
time from \citep[Phase~Redundancy]{lasser2026phase}.  (C11.CLK)
maps the wall-clock floor $\tau_{\mathrm{meta}}$ to an event-count
floor $\Nev(\tau_{\mathrm{meta}}) \geq \Ncasc$ with probability
$\geq 1 - \bclk$, supplying the comparable-clock chain $\Tbeta \leq
\Ncasc \leq \Nev(\tau_{\mathrm{meta}})$.  The lead-time-before-
cascade guarantee is therefore in event-count form: $\Tbeta \leq
\Nev(\tau_{\mathrm{meta}})$ with probability $\geq 1 - \bclk -
\bcal$.
