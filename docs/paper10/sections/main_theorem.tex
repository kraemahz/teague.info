...
Bounded co-evolution does not require its own assumption block:
\citep[Theorem~1]{lasser2026foundations} derives the property as
a corollary of (C5.HOEFF), the upper exposure-rate cap (C7.RATE)
introduced as a sub-clause of (C7) below, and the verification
protocol's channel-projection structure under (C5.MULT). The
companion paper establishes the explicit closed form $\bar{M}
\leq C \cdot \Bclip \cdot \lambda_{\max} \cdot \tau_{\mathrm{meta}}$
with $C \in \{1, \Kch - 1\}$, all factors deployment-class
constants intensive in $|P|$.
...
rather than from a separate empirical-adequacy assumption.
Concretely, \citep[Theorem~2]{lasser2026foundations} gives
\[
  |g(\Ttruth) - g(\Pproxy)| \;\leq\; \mathrm{Lip}(g) \cdot
  h_{\mathrm{CG}}, \qquad
...
auditable per \citep[\S 6]{lasser2026foundations}.  These replace
the previous monolithic HHI surrogate-adequacy assumption; the
proof of the bound under (REP) + (DISP) + (COAL) plus embedding
compatibility is in \citep[Theorem~2]{lasser2026foundations}.
The universal model-free Concentration-Gap conjecture remains
open, but the deployment claim no longer depends on it directly.
