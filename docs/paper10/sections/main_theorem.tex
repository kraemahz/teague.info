...
three-layer deployment-safety theorem composing the nine lemmas of
...
\begin{assumption}[Bounded co-evolution]
\label{ass:bounded_coev}
The deployment $\mathcal{D}$ operates in a regime where co-evolution
correction terms are bounded uniformly: there exists a constant
$\bar{M} \geq 0$ depending on $\mathcal{D}$'s operational parameters
but independent of $|P|$, such that the maximum per-channel coupling
magnitude satisfies $M(\mathrm{deployment}) \leq \bar{M}$ for all
valid deployment states.  Combined with the structural co-evolution
constant $K_{\mathrm{coev}} \geq 0$, the co-evolution positive-part
error term satisfies $(\epsnonresCoev)_+ \leq K_{\mathrm{coev}}
\cdot \bar{M}$.

This assumption is new to this paper.
\citep[Microfoundation]{lasser2026micro}'s Composition Proposition
establishes exact composition in the sequential-intervention regime
but explicitly leaves the simultaneous co-evolution correction
terms as open work; (C7) closes the gap operationally rather than
structurally, with empirical testability noted in
\S\ref{sec:tooling_substrate_audits}.
\end{assumption}
...
\item[(C1)] \textbf{Invariants in force.} The eleven operational
  invariants $I_1, \ldots, I_5, I_6', I_7, I_8, I_9, I_{10}, I_{11}$
  hold throughout the deployment window $[0, T]$. Additionally,
  Assumption~SA1 (HHI surrogate adequacy,
  \ref{ass:SA1}) holds so that $I_5$ is operationally meaningful for
  the theorem's Layer~1 binding.
...
\item[(C3)] \textbf{Cooperative-overlap regime.} The substrate-
  cooperative structure is dominated by cross-substrate cooperatives
  (the canonical tripartite case) rather than redundant capabilities,
  verified by the cooperative-vs-redundancy audit of
...
\item[(C5.SPRT)] \textbf{SPRT applicability (agent-side and
  ...
  \item[\textbf{(C5.MULT)}] \textbf{Bounded channel multiplicity,
    fixed before deployment.}  The monitored partition uses
    fixed-cardinality $\Kch$ channels with familywise
    $\alpha/\beta$ allocation; the channel-selection / testing
    policy is fixed before deployment.  Adaptive creation of new
    monitored channels falls into residual class~(R3).
  \item[\textbf{(C5.IID)}] \textbf{Adapted increments with
    ...
    clipping).
  \item[\textbf{(C5.SUPP)}] \textbf{LLR finiteness (with environment-
    side support conventions).}  Almost-sure finiteness of $\ell_n$
    holds automatically under (C5.HOEFF) clipping.  For
    environment-side monitors (Lemma~\ref{lem:5e}), per-variable
    distributions $p_{0, v}$ have support conventions:
    Bernoulli baselines $p_0 \in (\eenv, 1 - \eenv') \subset (0,
    1)$ open; Poisson baselines $\lambda_0 \geq \eenv > 0$, with
    deployment-class regularization constant $\eenv$.  Non-
    overlapping support is handled via clipping per (C5.HOEFF).
  \end{itemize}
...
\item[(C7)] \textbf{Bounded co-evolution.}
  Assumption~\ref{ass:bounded_coev} holds: the deployment's
  per-channel coupling magnitudes are uniformly bounded by some
  $\bar{M}$ independent of $|P|$.  This closes the co-evolution
  correction-term bound that
  \citep[Microfoundation]{lasser2026micro}'s Composition
  Proposition leaves open in the sequential-vs-simultaneous regime
  distinction.
\item[(C8)] \textbf{Per-capability admissibility.}  For every
  capability $c \in P^{\mathrm{act}}$, the per-capability proxy-
  truth gap is allocated a ceiling with disjoint per-channel
  sub-shares, in the sense of
  \citep[Microfoundation]{lasser2026micro}'s Assumption~1.  This is
  the precondition under which the four-channel decomposition's
  sup-norm is bounded by per-channel admissibility ceilings.
\item[(C9)] \textbf{World-model parameterization regularity.}
  ...
  parameterization choice (BD, CL, WB) and the operational active-
  subspace selection (TF).  See \S\ref{sec:tooling_substrate_audits}
  Audit~5 for the calibration procedure.
...
\item[(C11)] \textbf{Bounded gap-growth rate.}  For any adversarial
  strategy $q \in \Aadv \cup \Aadv^{\mathrm{env}}$ producing
  channel deviations bounded by $\deltastar$ (the post-clipping
  union-class drift floor under (C5.IID)), the per-SPRT-step
  proxy-truth gap growth is bounded:
  \[
    (\Delta \epsgap)_n := \epsgap(s_n) - \epsgap(s_{n-1})
    \;\leq\; \rhogap(\deltastar),
  \]
  where $\rhogap(\deltastar)$ is a $|P|$-independent deployment-
  class constant.  This is a Paper~10 operational assumption
  (analogous to (C7) bounded co-evolution and SA1 HHI surrogate
  adequacy), used in Lemma~\ref{lem:6} to bound the detection-window
  supremum gap.  See \S\ref{sec:tooling_substrate_audits} Audit~7 for
...
+ \rhogap(\deltastar) \cdot \Tbeta$ is intensive in $|P|$.  Total
...
$|P|$ or with the system's capability count. The guarantee scales
past any capability level the deployment's evaluation methods can
characterize, conditional on the invariants holding and the
deployment falling outside the four named residuals.
\end{theorem}
...
\emph{Step 3.} Under invariants $I_1$ and $I_4$, the contraction
...
\emph{Step 4.} Under invariant $I_5$ ($\HHI < H^*$) and Assumption
SA1 (\ref{ass:SA1}, HHI surrogate adequacy), the deployment lies
outside Conjecture~1's predicted-exploitation regime
$\mathcal{R}_{\mathrm{press}}$. SA1 is the explicit empirical
conditional: $\HHI < H^* \implies \mathcal{R}_{\mathrm{press}}^c$.
Lemma~\ref{lem:3_forward} establishes only the forward direction
($\HHI > H^*$ implies the prerequisites of
$\mathcal{R}_{\mathrm{press}}$); the reverse direction needed here
is supplied by SA1 as an explicit conditional, not as a derived
consequence. The proxy-truth gap is therefore bounded by its
non-residual component plus the residual floor:
\[
  \epsgap \;\leq\; \epsnonres + \lambda \cdot \epsfloor,
\]
where $\lambda \in [0,1]$ is the residual weight from the Microfoundation paper~\citep{lasser2026micro}
\S residual\_class.
...
\]
Define $h_{\mathrm{static}}(\theta) := f(\epsilon_{\mathrm{safe}}) +
\lambda \epsfloor$, where $\epsilon_{\mathrm{safe}}$ and $\epsfloor$
are determined by the invariant thresholds $\theta$.
...
$X = f(\epsilon_{\mathrm{safe}})$ (intensive by Lemma~\ref{lem:2}
under conditions added there): $h_{\mathrm{static}}(\theta)$ is
intensive in $|P|$, and the Layer~1 bound is intensive in capability
magnitude.
...
The four residuals (R1)--(R4) of the theorem statement are real,
...
\emph{(R3) Redundancy-dominated regime.} Lemma~\ref{lem:5c}'s
safe region is established for the cooperative-overlap regime.
Deployments where redundant capabilities dominate cross-substrate
cooperatives violate (C3); the equal-substrate analysis is
anti-conservative in this regime, and the safe region inequality is
not directly applicable. Sequential counterfactual derivation
(Lemma~5c-prime) is open follow-up work; until completed, redundancy-
dominated deployments cannot invoke Layer~1.
...
\textbf{Existence of a provably safe regime.} Under the eleven
invariants, the canonical tripartite substrate identification, the
cooperative-overlap regime, and the causally-grounded cooperative-
outcome inner-alignment condition, deployment of capability-unbounded
systems is provably safe in a defined sense (Goodhart slack
intensive in $|P|$, with tail-bounded detection of evasions in the
named class). The regime is not vacuous: \S\ref{sec:worked_scenarios}
walks through realistic deployments meeting all conditions.
...
\textbf{Named residual structure.} The four residuals are explicit
...
\paragraph{Conjectural assumptions (argued, not proved).}
The theorem does \emph{not} prove SA1's reverse direction
($\HHI < H^* \implies \mathcal{R}_{\mathrm{press}}^c$); SA1 is named
as an empirical assumption with a structural follow-up
(channel-mediation conjecture, \S\ref{sec:disc_companion}) and an
empirical validation pathway (\S\ref{sec:sa1_empirical}).
...
in (R1)--(R4) are explicitly outside the guarantee; we make no
...
(C1)--(C5)).  Establishing basin entry is open follow-up work.
