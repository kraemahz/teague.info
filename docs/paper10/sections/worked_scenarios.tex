...
(C1)--(C12) and Assumption~\ref{ass:concgap_conditions} are
checkable in concrete cases, not just in the abstract.
...
\emph{$\mindep$ audit (Audit 1).}  The threat model $\mathcal{T}$ enumerates:
(1) prompt-injection on the LLM, (2) human cognitive bias / motivated
reasoning, (3) cryptographic-implementation bugs, (4) training-data poisoning
of the LLM, (5) trusted-setup failure, (6) social engineering (excluded as R4
per \S\ref{sec:tripartite}). Each remaining mechanism is contained in one
substrate; $\mindep = 3$ verified.

\emph{Cooperative-vs-redundancy audit (Audit 3).}  Capabilities are
categorized: \emph{Single-substrate} (76 capabilities), \emph{Cross-substrate
cooperative} (24 capabilities including the verified-research-output
cooperative spanning all three substrates), \emph{Redundant} (3 capabilities,
the redundant ledger replicas). $\volL$-weighted: $V_{\mathrm{coop}} = 14.2$,
$V_{\mathrm{redun}} = 0.4$. $V_{\mathrm{coop}} \gg V_{\mathrm{redun}}$, so the
deployment is in the cooperative-overlap regime.
...
\textbf{Theorem~1 binding.}  All twelve theorem conditions plus
the Concentration-Gap structural conditions
(Assumption~\ref{ass:concgap_conditions}) hold throughout the quarter.  Core
regime conditions (C1)--(C5):
\begin{itemize}
...
\item Assumption~\ref{ass:concgap_conditions} structural conditions
  ((REP), (DISP), (COAL), (EMB)) certified by audit per
  \citep[\S 6]{lasser2026foundations}: counterparty-population
  diversity verified, dispersion estimate $\sigma$ within
  deployment-class bound, coalition-detection produces no flagged
  blocs above audit threshold, embedding $\phi$ certified
  affine-isometric on the operationally active subspace.
\end{itemize}
Deployment-class conditions (C5.SPRT) and (C6)--(C12) are calibrated
via Audits 4--8 (\S\ref{sec:tooling_substrate_audits}):
\begin{itemize}
\item (C5.SPRT) sub-clauses (HOEFF/MULT/IID/SUPP) calibrated via
  Audit~7: $\Bclip$, $\Kch \leq 8$, multinomial-cardinality bound
  $\Kchmulti$, post-clipping $\deltastar$, and multinomial support
  floor $\eenv$ all certified.
\item (C6) bounded-Lipschitz alignment verified: the deployment's
  welfare functional $g$ is a bounded-weight scalarization
  ($K_{\mathrm{Lip}}$ certified).
\item (C7) bounded co-evolution: $\bar{M}$ certified by per-channel
  coupling audit (Audit~4).
\item (C8) per-capability admissibility ceilings: assigned by
  policy and verified per capability.
\item (C9) parameterization regularity (BD/CL/WB/TF) verified via
  Audit~5.
\item (C10) pairwise cooperative-production floor with audited
  subset $\Saudit$ certified via Audit~6 ($\rho_0 > 0$).
\item (C11) bounded gap-growth rate $\rhogap$ certified via Audit~7
  as a uniform worst-case bound.
\item (C11.CLK) clock comparability: $\Ncasc \geq \Tbeta$ certified;
  $\bclk, \bcal$ documented.
\item (C12.ENV-WIT) environment-side witness instantiation certified
  via Audit~8 (PUB/PART/TRUST-WRITE/TRUST-CORR/SETUP/CAL).
\end{itemize}
...
\paragraph{Violation 1: \texorpdfstring{$I_1$}{I1} ($\rhomincross$ floor) collapse.}
...
\paragraph{Violation 2: \texorpdfstring{$I_3$}{I3} (subsumption frequency) burst.}
...
\paragraph{Violation 3: \texorpdfstring{$I_5$}{I5} (HHI ceiling) breach.}
...
\paragraph{Violation 4: \texorpdfstring{$I_9$}{I9} (substrate-exclusivity observability) failure.}

\emph{Symptom.}  An audit reveals that two of the three ledger nodes are
operated by the same cloud-provider failure mode (a recently-discovered shared
dependency). Substrate-exclusivity has degraded without producing visible
signals on the four monitored channels.
...
\paragraph{Violation 5: \texorpdfstring{$I_{10}$}{I{10}} (coverage/materiality) violation.}
...
\paragraph{Violation 6: \texorpdfstring{$I_{11}$}{I{11}} (latency bounds) violation.}
...
\emph{Defense (cooperative-anchoring + $I_9$).}  The human substrate
participates in the specific shared verification infrastructure committed to
the ledger. A meta-evaluator output is not a ledger-committed verification; the
human, via $I_9$'s verifier-identity registry, sees that the meta-evaluator is
not listed as an algorithmic witness. The cooperative-anchoring property's
Component~3 (output non-fungibility): the meta-evaluator's verdict is not the
same as a witness's verdict; the cooperative output is undermined.
...
\item Cryptographic-implementation bugs in the witness. Single-substrate (Formal-Operational).
\item Training-data poisoning. Single-substrate (AI).
\item Trusted-setup ceremony failure. Single-substrate (Formal-Operational).
...
\textbf{$\alpha$-distribution audit (Audit 2).}  Compute counter-factual loss fractions:
...
The deployment specifies parameters from which the operator
computes per-channel KL floors via the formulas in
Appendix~\ref{app:proof_lem5b}:
\begin{itemize}
\item Channel 1 (Poisson cooperative-rate): $\lambda_0 = 8$
  cooperative events per day, adversarial alternative $\lambda_1 =
  6$ (shift $\eta_1 = 2$).  Per the Poisson KL formula,
  $\delta_1 = 6 \log(6/8) - 6 + 8 \approx 0.274$.
\item Channel 2 (Bernoulli attestation): $p_0 = 0.95$, $p_1 = 0.85$
  (shift $\eta_2 = 0.10$).  Per the Bernoulli KL formula,
  $\delta_2 = 0.85 \log(0.85/0.95) + 0.15 \log(0.15/0.05) \approx
  0.070$.
\item Channels 3, 4 (multinomial concentration): per
  Appendix~\ref{app:proof_lem5b}, the least-favorable
  concentration on the smallest-baseline-mass cell gives positive
  deployment-class floors $\delta_3, \delta_4$ depending on the
  multinomial cardinality and support floor $\eenv$ from
  (C5.SUPP); illustrative deployment values $\approx 0.10$.
\item $\deltaadv = \min_c \delta_c \approx 0.07$ for this
  illustrative parameterization.
\end{itemize}

The numbers above are illustrative for a concrete deployment-tooling instance;
real deployments compute exact values via the formulas in
Appendix~\ref{app:proof_lem5b} from the deployment's chosen $\lambda_0, p_0,
\eta_c, \eenv$ values.
...
of (C1)--(C12) translates to a concrete audit, calibration, or
...
vacuous --- there exist deployments for which (C1)--(C12) and
Assumption~\ref{ass:concgap_conditions}'s structural conditions hold, and the
deployment-safety bound binds.
