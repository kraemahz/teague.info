...
pressure regime) was previously asserted as a single
empirical-adequacy assumption (HHI surrogate adequacy) the
deployment would condition on.

\subsection{The companion-paper discharge}

The companion paper~\citep{lasser2026foundations} discharges a
\emph{scoped} version of the Concentration-Gap~Conjecture and
replaces the monolithic HHI surrogate-adequacy assumption with
...
Layer~1 of Theorem~\ref{thm:deployment_safety} now binds via
this structural result rather than via a separate
empirical-adequacy assumption.
...
The companion paper's contribution is shifting the second
category (structural conditions) from monolithic
empirical-adequacy claims to operationally auditable
verification-protocol-design conditions.
