...
\item[(c)] \emph{Boundary residual.}  Identify
  ...
  \emph{boundary-crossing subsumption pairs} (pairs $c \succeq
  c'$ with $c, c'$ in different original cells), and assign
  the boundary cooperative nodes' $\volL$ weight together with
  any subsumption-crossing weight redistribution to a fifth
  bucket $\delta_{\mathrm{boundary}}$.  Cooperative nodes carry
  explicit $\volL$ weight ($w(c_{\mathrm{coop}})$) and migrate
  cleanly to the bucket; subsumption-crossing edges do not have
  standalone $\volL$ mass, so their contribution to
  $\delta_{\mathrm{boundary}}$ is the $\volL$ weight of the
  $\succeq$-coupled node-pair under M4-decomposition that fails
  for them in the cell partition---formally, the weight
  $w(c) + w(c')$ minus whatever contribution the cells
  separately accommodate, which is zero under the convention
  that subsumption-crossing pairs are placed wholly in the
  bucket.  The identity becomes
  $1 - \betaL = \delta_{\mathrm{restricted}} +
  \delta_{\mathrm{covered}} + \delta_{\mathrm{dormant}} +
  \delta_{\mathrm{residual}} + \delta_{\mathrm{boundary}}$ with
  the fifth term capturing all cross-cell poset mass.  Strategy
  (c) is what the worked example uses
