...
\end{align*}
Under poset-boundary closure, the identity
$1 - \betaL = \sum \delta$ follows from two additivity steps.
First, $\volL(P) = \volL(R) + \volL(C) + \volL(D) + \volL(S)$
by poset-boundary closure plus set-disjoint union via
...
$\volRlower$ by definition (dormant cells have no admissible
sacrifice events covering them; residual cells admit no
sacrifice-observable path even in principle).  Combining:
$1 - \betaL = 1 - \volRlower(P)/\volL(P) = [\volL(P) -
\volRlower(P)] / \volL(P) = \sum_X [\volL(X) -
\volRlower(X)] / \volL(P)$, which expands to the four-term
identity (with $\volRlower(D) = \volRlower(S) = 0$ giving
$\delta_D = \volL(D)/\volL(P)$ and $\delta_S =
\volL(S)/\volL(P)$ directly).
\end{definition}
...
\item[(c)] \emph{Boundary residual.}  Identify
  \emph{boundary-crossing cooperative nodes} (cooperatives in
  $P$ whose participants span two or more original cells) and
  \emph{boundary-crossing subsumption pairs} (pairs
  $c \succeq c'$ with $c, c'$ in different original cells).
  Cooperative nodes carry explicit $\volL$ weight
  $w(c_{\mathrm{coop}})$ from M5; assign that weight to a
  fifth bucket $B$ (so $\volL(B) \supseteq \sum_{z \in
  \mathrm{cross-cell\ coops}} w(z)$).  For
  subsumption-crossing pairs, the $\succeq$ relation does not
  carry standalone $\volL$ weight; we handle them by
  \emph{relocating one endpoint into the fifth bucket}.
  Specifically, for each cross-cell subsumption pair
  $(c, c')$, move whichever endpoint has lower priority (or
  fewer in-cell connections, by tie-break) into~$B$; this
  removes the cross-cell edge from the cell partition (the
  endpoint is no longer in its original cell, so the edge no
  longer crosses cells) and contributes that endpoint's
  $w$-weight to $\volL(B)$.  After this relocation step, the
  cells $\{R, C, D, S, B\}$ are pairwise poset-independent in
  the same sense as Definition~\ref{def:gap_decomp}, M4
  applies across them, and the identity becomes $1 - \betaL =
  \delta_{\mathrm{restricted}} + \delta_{\mathrm{covered}} +
  \delta_{\mathrm{dormant}} + \delta_{\mathrm{residual}} +
  \delta_{\mathrm{boundary}}$ with all five terms now well-
  defined as $\volL$-fractions.  Strategy~(c) is the variant
  the worked example uses (Section~\ref{sec:worked_example}).
\end{enumerate}
