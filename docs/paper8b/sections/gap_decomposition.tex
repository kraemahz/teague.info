...
\volL(S) + \volL(B)$ by literal atomic-weight summation
$\volL(X) := \sum_{c \in X} w(c)$ across the
...
\item[(c)] \emph{Boundary residual (literal-partition
  ...
  by the strategy-(c) atomic-weight accounting convention.  Subsumption-crossing
...
cost of a weaker (atomic-weight accounting only) additivity
license.  Strategies (a)
...
\emph{objective} side, asserting that $\Delta\volL(X)$ is
correctly computed from the surrendered benchmarked capability
via the public benchmark function, is verified by the commitment
layer's ZK proof on the magnitude commitment; (ii)~the
\emph{subjective} side, asserting that the agent's valuation
$\Ui(X)$ of the surrendered capability matches $\Delta\volL(X)$
