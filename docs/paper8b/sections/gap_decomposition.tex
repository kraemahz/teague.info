...
\item \textbf{Alarm fires and $\delta_{\mathrm{covered}}$
  ...
  \volL(G)$ is large, indicating
  $\volRlower(C) \ll \volL(C)$).  This is the canonical
  proxy-failure signature: capabilities the sacrifice channel
  \emph{can} reach are present in $\volL$ but registering little
  exercise evidence.
...
distinguish the regimes.  The converse failure mode---true
$\volR$ high but the sacrifice channel sees little
evidence---produces low $\betaL$ and would also fire the alarm,
making the alarm a false-positive proxy-failure signal in that
regime as well; this is the structural cost of
privacy-respecting observation, and the gap decomposition's
$\delta_{\mathrm{residual}}$ and $\delta_{\mathrm{restricted}}$
cells are precisely the diagnostic that distinguishes
``low-evidence-but-true-low-exercise'' from
``low-evidence-but-channel-blind.''  The alarm has no
\emph{false negative} in the conventional sense ($\betaL$ is
computed from observed events; the alarm fires deterministically
whenever the threshold is crossed), only the proxy-failure
false-positive modes flagged here.
\end{remark}
