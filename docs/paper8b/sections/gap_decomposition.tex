...
cell by stipulation; under that weaker semantics, the
identity follows from set-disjoint M5 weight summation alone
without the (CC1)/(CC2) hypotheses, but the cells are no
...
the event-local bundle decomposition~\citep{lasser2026paper8a}---accumulate the
...
\item[(b)] \textbf{Low $\volRlower$ + high $\NAC$:} Proxy
  ...
  Definition~\ref{def:s1_sufficiency}, so a low $\volRlower$
