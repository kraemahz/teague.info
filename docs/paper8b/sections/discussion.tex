...
wireheading-consistent concentration signal with
Schur-convexity as its formal monotonicity basis.  The Part-A
category-flow variant is third-party observable from public
committed sacrifice events
(Proposition~\ref{prop:third_party_wireheading}).

\paragraph{The residual class is structurally irreducible.}
...
observation time.  The residual class is the irreducible
component of the gap decomposition.  The revealed-observation
carve of $\ResS$ (as the set of capabilities
\emph{structurally outside the channel's reach}) differs from
and is operationally cleaner than the classical
benchmarkability-based carve.
