...
channel has not yet observed it.  Examples are restricted to
...
\item[(c)] Capabilities in $\ResS \setminus \ResB$: capabilities
  ...
  verifiable) but admit no sacrifice-observable exercise mode
  in principle.  Example: ongoing interoceptive self-awareness
  exercised concurrently with all other activity (the proof
  below works through this witness in detail).  Cognitive
  capabilities such as mathematical reasoning or strategic
  planning are typically \emph{outside} $\ResS$, not inside,
  because they admit paid-training, dedicated-time, or
  counterparty-supported exercise modes that produce sacrifice
  events; the proof below makes this distinction explicit.
...
(\citep{lasser2026paper8a}) on the underlying bundle.  Whether
...
satisfied here because each session admits an admissible
...
of the companion paper~\citep{lasser2026paper8a}.  Without the
