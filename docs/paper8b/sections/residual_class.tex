...
lies in neither $\ResS$ nor $\ResB$.  The genuine
$\ResS \setminus \ResB$ witness is the \emph{baseline}
component---the always-on background condition of conscious
embodiment that is benchmarkable in principle (one could
imagine a clinical baseline measurement, so $\notin \ResB$) yet
admits no incremental sacrifice path: payments and time-spend
that increase \emph{trained acuity} are sacrifices for the
trained-acuity capability, not for the baseline; the baseline
is exercised continuously as long as the agent is conscious,
with no incremental material input, time displacement, or
counterparty distinguishing the baseline from the conscious
default.  Under the conservative reading of $\ResS$
(Remark~\ref{rem:residual_conservatism}), the baseline
component is the genuine witness.  Similar structural arguments
apply to baseline (untrained) peripheral sensory integration
and baseline attentional stability, distinguished from their
trained/honed forms which lie outside $\ResS$.
