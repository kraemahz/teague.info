...
interpretation, more naturally aligned with sacrifice-channel
diagnostics: the channel-reach criterion is anchored in the
observation infrastructure's modality (what kinds of events the
channel emits, expressed as a structural claim about candidate
exercise pathways), while the benchmarkability criterion is
anchored in capability-individuation discipline.  Both criteria
require domain analysis to apply to specific capabilities (see
the structural/domain proof-obligation note in
Definition~\ref{def:residual_class} below); neither is
\emph{directly checkable} from trade records alone.  Whether the
revealed-sacrifice framing constitutes ``operationally cleaner''
...
Classifying a capability as residual should be conservative.
``Plausible mechanism'' is itself contestable: ``private
contemplative experience'' might displace idle cycles that have
time-opportunity cost, ``baseline'' awareness capabilities
might be measurable through clinical assessment that costs the
agent's time.  Classify $c \in \ResS$ only when every proposed
sacrifice path can be shown to belong to a \emph{distinct
capability} (e.g., the act of being assessed is a distinct
capability from the assessed underlying state) under the
framework's poset-construction discipline.  This shrinks
$\ResS$ relative to a looser reading; a smaller residual class
makes the gap-decomposition's residual cell smaller, which is
...
sacrifice paths attach to distinct capabilities, not the
target $c$ itself.  When such arguments are absent, classify
$c$ as non-residual by default.
...
\item[(d)] Whether $\ResB \cap \ResS$ is non-empty is
  \emph{individuation-conditional} in the same sense as (c):
  candidate witnesses (private contemplative experience with no
  material input or time-sacrifice signal) lie in
  $\ResB \cap \ResS$ under poset-construction choices that
  treat ``idle'' time as not opportunity-cost-attributable;
  under a stricter individuation that attributes opportunity
  cost even to idle intervals, the witness migrates out of
  $\ResS$.  See (d) of the proof for the conditional witness.
\end{enumerate}
The proposition's downstream uses in this paper require only
(a) and (b); (c) and (d) are presented as
individuation-conditional witnesses for completeness, and
$\ResB \cap \ResS$ is reported as conditionally non-empty
rather than unconditionally non-empty.  The classical and
channel-reach residual carves therefore overlap conditionally
(witnessed by (d) under the stipulated individuation) but are
not in a containment relationship in the strict direction
$\ResB \subseteq \ResS$ (witnessed by (b)); the converse
direction $\ResS \subseteq \ResB$ is not asserted by this paper.
\end{proposition}
...
$\ResS \setminus \ResB$ witness.  Similar candidate witnesses
(subject to the same individuation assumption) include
baseline peripheral sensory integration and baseline
...
even to ``idle'' time, the witness fails.  This makes (d)
conditional in the same sense as (c).  The proposition's
downstream uses (Section~\ref{sec:gap_decomposition}'s
residual cell) only require $\ResS$ to be
\emph{constructible}, not non-empty;
$\ResS = \emptyset$ would just collapse
$\delta_{\mathrm{residual}}$ to zero.
\end{proof}
...
privacy commitments: the question is ``what do we actually
see?'' rather than ``what could we test if we had access?''
Other carves (benchmarkability, computability) remain
appropriate for other questions.
\end{remark}

\subsection{Bounding the residual class}

GFM's observational-individuation
...
$\volRexact^{[W]}(O)$.  By
the Horizon-Aware paper's~\citep[Corollary~2.1]{lasser2026horizon},
...
Corollary~2.1~\citep[Horizon~Aware]{lasser2026horizon} establishes only the
...
would double-count.  When boundary-closure fails (either BC1 or BC2), the
compositional lower-bound interpretation of $\volRcomb$ is
\emph{not established}---per the boundary-cooperative-undercount
remark in~\citep[Revealed~Sacrifice]{lasser2026paper8a},
$\volRcomb \leq \volR(P)$ holds only under BC1 + BC2; under BC1
failure $\volRcomb$ undercounts boundary cooperatives, under BC2
failure the sign of the gap is undetermined.  In either failure
regime $\volRcomb$ can still be computed and reported as a
heuristic composite of the two sub-diagnostics but should not
be read as a certified lower bound on $\volR(P)$.  The OI floor's
contribution to $\volRexact(O)$ is unaffected by either failure
mode because the OI capability $d_k$ is, by construction,
an internal capability of agent $k$'s substrate---not a
cooperative or subsumption edge spanning the perimeter
boundary---so the OI floor remains a valid lower bound on
$\volR(O)$ regardless of BC1/BC2.
\end{remark}
