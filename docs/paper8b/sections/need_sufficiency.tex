...
\item \textbf{Above sufficiency on all dimensions:} The agent
  has reached functionality and is choosing to invest further.
...
right-continuous at every $s \in [0, 1]$ (so the function has
no jump discontinuities approached from the right; in
particular $\lim_{s \to 0^+} g(s) = \alpha_0$).  Right-continuity
everywhere is what excludes step-function gates with abrupt
jumps in the interior of $[0,1]$; the boundary-only condition
$\lim_{s \to 0^+} g(s) = \alpha_0$ alone would not suffice.  A
gate with $\alpha_0 = 0$ is called a
...
alternative actions or trades available.  S1 fails for a
sacrifice event $(i, X_n, Y_n, t_n)$ when both:
\begin{enumerate}
...
\item[(H2)] no alternative in $\mathcal{A}_i$ other than the
  ...
  trade})\}$, taking action $a$ leaves the agent below $s_k$ on
  some need dimension.
\end{enumerate}
Under (H1) and (H2), no non-degrading alternative-to-the-trade
exists in $\mathcal{A}_i$, S1's non-degrading-alternative
condition fails, and the event is filtered.  We refer to
events satisfying (H1)+(H2) as \emph{below-sufficiency
sacrifices}.  Below-sufficiency \emph{targeting}
(sacrifice aimed at improving an unmet need dimension where
(H1) holds but (H2) fails because $\mathcal{A}_i$ contains a
non-degrading non-trade alternative such as a different trade
or non-trade action) does not by itself imply S1 failure: S1
can still hold via that third alternative, in which case the
trade reveals genuine preference between non-degrading options.
\end{proposition}
