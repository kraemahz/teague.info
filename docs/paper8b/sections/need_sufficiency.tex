...
cost.  Both quantities are dimensionless ratios in
cost-reference units, so $\mathrm{actual}_k$ and $s_k$ live on
the same scale and benchmark maximization aligns with cost
minimization.  The inversion is defined on
...
theorem~\citep{lasser2026paper8a}).  Below-sufficiency
...
Let $(i, X, Y, t)$ be a sacrifice event satisfying the
proposition's hypotheses: retention of $X_n$ leaves the agent
below threshold $s_k$ on need dimension $d_k$, and no third
alternative exists in the agent's choice set that is
non-degrading on every need dimension.

S1 requires that some alternative to making the trade is
\emph{non-degrading}: it must not drop the agent below minimum
viable functionality on any need dimension.  S1's alternative
ranges over the agent's choice set, which we partition into
two classes: (a)~the retention alternative (refuse the trade,
keep $X_n$), and (b)~any third alternative (a different trade,
or a different non-trade action).

By the first hypothesis, retention drops the agent below $s_k$
on $d_k$, so retention is degrading and class~(a) fails the
non-degrading-alternative condition.  By the second hypothesis,
no element of class~(b) is non-degrading on every need
dimension, so class~(b) also fails.  Together, no
non-degrading alternative exists in the agent's choice set, so
S1's condition fails for this event.  Events with failed S1
are excluded from the aggregate by the applicable-assumptions
clause of the aggregate lower bound
theorem~\citep{lasser2026paper8a}.

The second hypothesis is what distinguishes ``below-sufficiency
sacrifice'' (S1 fails) from ``below-sufficiency targeting''
(S1 may still hold via a third alternative): the latter is
consistent with S1 because the agent could choose a different
non-degrading option, in which case the trade is not forced
and reveals genuine preference.  In practice the second
hypothesis must be established by the institutional
attestation layer of the companion
paper~\citep{lasser2026paper8a} (the external-attestation
...
alone), not derived structurally from sufficiency status
alone.
\end{proof}
