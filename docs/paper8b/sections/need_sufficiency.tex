...
Let
$\Pi_N^0 = \{\pi : d_k(\pi) = s_k \text{ for all } k\}$
be the class of pathways satisfying each dimension of~$N$ at
exactly its sufficiency threshold, assumed non-empty by
governance realizability hygiene.  In discrete-valued domains
where exact threshold-achievement may be infeasible, $\Pi_N^0$
is relaxed to pathways within a domain-set tolerance of the
threshold; the framework's downstream arguments are preserved
by treating the relaxed set as the admissible reference base.  Let
...
threshold-level pathways against which downstream
non-degradation is evaluated.  Need $N$ at position $p_N$ is
...
\item[(b)] \textbf{Downstream non-degradation.}
  For each downstream capability $c \in \Down(p_N)$, let
  $\mathrm{quality}_c : \Pi_N^0 \cup
  ...
  the standard-of-care quality function on care bundles; for
  water access, the contamination-adjusted access function).  The framework
  ...
  Write $\mathrm{quality}_c[\pi]$ for the quality of capability
  $c$ when need~$N$ is satisfied via pathway~$\pi$.  Then
  $\pi_N$ must dominate \emph{every}
  $\pi^* \in \Pi_N^{\mathrm{ref}}$ on every downstream
  capability simultaneously:
  \[
    \mathrm{quality}_c[\pi_N] \;\geq\;
    \mathrm{quality}_c[\pi^*]
    \quad\text{for all } c \in \Down(p_N),
    \;\pi^* \in \Pi_N^{\mathrm{ref}}.
  \]
\end{enumerate}
...
to a risk claim under~\citep[Controlled~Relaxation]{lasser2026wamura}.  A failing risk
...
Below-sufficiency sacrifices in the H1+H2 narrow sense of
Definition~\ref{def:s1_sufficiency}
...
automatically filtered by this definition: such events may
...
sequence.  Wireheading
(\citealp[Discussion]{lasser2026exo}) is narrow
self-reinforcing exercise of existing capabilities, whereas
stuffing adds new capabilities without exercising them
degenerately.  The Doll Problem
(\citealp[Section~7.1]{lasser2026gfm}) concerns
possessed-but-unexercised capabilities, whereas stuffed
capabilities may be technically exercised (routing through the
gas station ``exercises'' the distance-measurement).  The
B-to-C gap in the aggregate sense this paper otherwise treats
is widened by stuffing through a specific mechanism
(individual-weight inflation) that the aggregate lower
bound theorem~\citep[Revealed~Sacrifice]{lasser2026paper8a}
does not, by itself, address.  Stuffing is a framework-native
perverse instantiation: the optimizer discovers it by following
the $\volL$-maximization gradient under M5, not by departing
from the framework's rules.
...
The GFM commitment protocol for timestamped claims, the
companion paper's sacrifice channel~\citep[Revealed~Sacrifice]{lasser2026paper8a}
for partial use-evidence, and risk controlled relaxation as an
...
Sufficiency is approximated by the level at which sacrifice
polarity statistically flips in aggregate.  As a
\emph{transition-region heuristic}, observed sacrifice trends
toward involuntary, retention-degrading-and-no-alternative
patterns below the threshold and toward
voluntary, choice-driven patterns above it; the transition
point in the sacrifice data approximates the threshold
empirically.  This is the polarity-classification statistic
on aggregate behaviour, not the per-event S1 admissibility
rule itself: per-event S1 classification follows
Definition~\ref{def:s1_sufficiency}'s H1+H2 condition (which
distinguishes structurally-diagnosable below-sufficiency
S1 failure from below-sufficiency targeting with a third
non-degrading alternative), and the
revealed-sacrifice transition region serves as a calibration
input rather than a substitute for that rule.  This mechanism
also has a bootstrap problem: identifying the threshold
requires classifying events, but classifying events requires
the threshold.  This
...
breaks down or sacrifice polarity inverts, which are
empirical or structural facts about agents in the deployment
context.
