...
(\citep{lasser2026paper8a}) assumes all sacrifices have
...
The inversion is defined on $\mathrm{cost}_k > 0$; the
zero-cost limit is read as ``no cost-dimension constraint
binds'' and treated as benchmark-saturated rather than via
$1/0$ (operationally: cap $1/\mathrm{cost}_k$ at a domain-set
ceiling above $s_k$, or apply a saturating transform such as
$\mathrm{cost}_k / (\mathrm{cost}_k + \epsilon)$ inverted on
$\mathrm{cost}_k + \epsilon$ for a small floor $\epsilon > 0$).
The choice of regularisation is a domain parameter; this paper
adopts the cap-at-ceiling convention by default and notes the
saturating-transform alternative for cost dimensions where
$\mathrm{cost}_k$ approaches zero in normal operation.
\end{definition}
...
its sufficiency threshold (assumed non-empty).  Let
...
\item[(b)] \textbf{Downstream non-degradation against all
  admissible references:} $\pi_N$ dominates \emph{every}
  $\pi^* \in \Pi_N^{\mathrm{ref}}$ on every downstream capability
  simultaneously:
  \[
...
mode of Assumption~S1 (\citep{lasser2026paper8a}) is
...
\item \textbf{Above sufficiency:}
  ...
  per-unit-of-improvement contribution near sufficiency is
  $\alpha_k$ above versus $1/s_k$ below, so closing the
  sufficiency gap is more valuable per unit of improvement than
  enhancing beyond sufficiency.  This is a marginal-rate claim,
  not an absolute-magnitude claim: because the above-sufficiency
  term is unbounded in $\mathrm{actual}_k$, sufficiently large
  above-sufficiency gains can dominate the closed-gap benchmark
  in absolute terms.  Bounding total above-sufficiency
  contribution requires either a domain-specific cap on
...
The rewighting is compatible with M5 (each capability still
contributes a non-negative weight) and with M4 on the
poset-independent direct sum (the gate factor depends on
$s_N$, not on poset position, so additivity over
poset-independent components transfers from $\volL$ to
$\volL^{\mathrm{eff}}$).  M6 (cooperative superadditivity) is
preserved when cooperative capabilities themselves carry
cooperative-node weights $w(c_{\mathrm{coop}})$ that are
gated alongside their participants; the cooperative term is
gated independently of (and multiplicatively with) its
participants' gates because $\mathcal{N}(c_{\mathrm{coop}})$
is computed for the cooperative node itself.
$\volL^{\mathrm{eff}}$ is therefore a reweighting of the same
$\volL$-sum, not a new measure with its own axioms; the
...
S1 fails for any sacrifice event in which the sole non-trade
alternative consistent with the agent's choice set is to retain
the sacrificed capability $X_n$, and retention would leave the
agent below the sufficiency threshold on at least one need
dimension.  Concretely: if for some need dimension $d_k$ and
threshold $s_k$, retaining $X_n$ produces capability level
below $s_k$ on $d_k$ \emph{and} no other alternative in the
choice set is non-degrading on every need dimension, then no
non-degrading retention alternative exists, S1's
non-degrading-alternative condition fails, and the event is
filtered.  We refer to events in this regime as
\emph{below-sufficiency sacrifices}.  Below-sufficiency
\emph{targeting} (i.e., sacrifice aimed at improving an unmet
need dimension) does not by itself imply S1 failure; S1 fails
specifically when retention would leave the agent
below-sufficiency \emph{and} no third alternative exists.
\end{proposition}
...
(the aggregate lower bound theorem~\citep{lasser2026paper8a}).  The sacrifice channel
...
paper~\citep{lasser2026paper8a}) remain.
\end{remark}
...
the aggregate lower bound theorem~\citep{lasser2026paper8a}.
\end{proof}
...
the aggregate lower bound theorem~\citep{lasser2026paper8a} does not, by itself, address.
...
companion paper's sacrifice channel~\citep{lasser2026paper8a}
