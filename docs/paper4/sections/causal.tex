...
\item \textbf{Capability gating.} Papers 2 and 3 of the GFM sequence
    ...
    robustness hierarchy: capability gating is nearly unfakeable
    \emph{relative to a trusted, sufficiently complete capability
    graph}, because it reflects the structure of the poset itself
    rather than behavior an adversary can stage. (Hidden delegation,
    latent substitute capabilities, or a misspecified poset can
    defeat the gate; the strength of the gate is bounded by the
    fidelity of the maintained capability model.) An attribution
    decision that conflicts with capability gating should be
    overridden by it, not aggregated with it.
