...
When the variance-reduction results of this paper are invoked
    ...
    equations are mutually independent conditional on $G_t$. That is,
    if $U_j \subseteq U$ are the exogenous variables appearing in
    $a_j$'s structural equation and $U_k \subseteq U$ those appearing
    in $a_k$'s, then $U_j \perp\!\!\!\perp U_k \mid G_t$ for $j \neq k$.
    This rules out shared exogenous shocks that would induce
    covariance between agents' contributions even under a correct
    causal DAG.
...
(c) Under the additive-SCM assumption
...
and the residual $h(G_t, U)$. By the variance identity for sums:
\[
\sigma^2 = \mathrm{Var}(\CausalAttr(a_j))
    + \mathrm{Var}(\mathrm{confounders})
    + 2\,\mathrm{Cov}(\CausalAttr(a_j), \mathrm{confounders}).
\]
Under causal sufficiency, the additive-SCM structure, and
exogenous orthogonality (Definition~\ref{def:scm}): causal
sufficiency ensures the DAG captures all common causes, the
additive decomposition ensures $a_j$'s contribution enters as a
separable term $g_j$, and exogenous orthogonality ensures the
noise driving $g_j$ is independent of the noise driving the
confounder terms $\{g_k\}_{k \neq j}$ and $h$. Together, these
give
