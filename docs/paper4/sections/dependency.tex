...
\end{equation}
This gap is positive whenever $p_{\mathrm{adv}} > 0$, the population
    satisfies substrate capability non-redundancy
    ...
    $\mathcal{S}_i$'s share of total volume.
\end{proposition}
...
($c_i \to 1$) on the targeted substrate. Smaller substrates in general
reach sigmoid saturation faster than larger ones because contagion
propagation time scales sub-linearly with population size, so it is
standard to expect $c_i > c$ for the substrates in a diversified
partition. The gap remains positive as long as each substrate's volume
