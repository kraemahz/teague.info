...
Let $X_j(t) = \|r_j^{\mathrm{risk}}(t)\|^2$ and assume $a_j$'s risk-claim
    ...
    $\nu_j^{\mathrm{risk}} = \mathrm{Var}[X_j] < \infty$, and that the
    squared-residual sequence is i.i.d.\ (or weakly dependent with
    summable autocovariance). Then the cumulative inconsistency estimator
    satisfies:
\begin{align}
...
\end{align}
Consequently $\Trisk_j(t)$ concentrates around
    $\Trisk_j^* = 1 / (1 + \beta_{\mathrm{risk}} \mu_j^{\mathrm{risk}})$
    in mean, with stationary deviation controlled by
    $\alpha_{\mathrm{risk}}$. Structurally accurate reporters (small
...
\mathrm{Var}[\sigma_j^{\mathrm{risk},2}(t)]
    ...
    &\longrightarrow \frac{(1 - \alpha_{\mathrm{risk}})^2}
       ...
       \cdot \nu_j^{\mathrm{risk}},
\end{align*}
giving Equation~\eqref{eq:risk_trust_variance}. Under weak dependence
with summable autocovariance, the same limit holds up to a
multiplicative constant reflecting the autocovariance sum. The
concentration statement for $\Trisk_j(t)$ follows by applying the
continuous mapping theorem to
...
The companion paper \citep{lasser2026horizon} sketches
$\hat{\Risk}_j(S; a_k, t) = T_j^{\mathrm{risk}} \cdot p \cdot
|\Delta \volL|$ as a product of a probability and a contraction
magnitude --- that is, an expected-contraction object in the same
units as $\volL$, not a probability in $[0, 1]$. Taking a log-odds of
that object is not well-defined. While integrating the aggregation
rule with the rest of this paper we noticed this inconsistency, and
we commit here to the definition update that makes the aggregation
work: $\hat{\Risk}_j$ and $\Risk_{\mathrm{self}}$ denote the reporter's
and the actor's posterior probability that the exercise pathway will
be executed within the planning horizon, calibrated against the shared
prior $\pi_0$ introduced below. The contraction magnitude
$|\Delta \volL|$ remains a separate quantity: it is used by the
verification gate (Section~\ref{sec:attention_not_action}) and enters
the structural verification residual
(Definition~\ref{def:risk_residual}) independently of the
probability-aggregation step. We treat the resulting pair
$(\hat{\Risk}_j, |\Delta \volL_j^{\mathrm{claimed}}|)$ as the canonical
two-component form of a communicated risk claim for Paper~4 onward;
the single-number product form in the companion paper should be read
as the first-sketch version of this pair.
...
where each parenthesized term is the log-likelihood-ratio contributed
    ...
    being executed within the planning horizon. Under the neutral
...
knows the parameters, and the ROC curve is particularly unfavorable: false
positives (restricting a safe capability) and false negatives (ignoring a
real risk) both cause $\volL$-contraction. We therefore adopt the activation
...
reporters, trust-history patterns) is game-able: adversaries who know the
detection threshold can space their reports, diversify their trust-building
histories, and avoid clustering. The two-gate architecture avoids this arms
