...
The main-text proof (\S\ref{sec:channel_independence}) is now the
primary version and includes the full causal-path analysis with the
strengthened conditioning on $(\mathbf{C}_a, B_1, \ldots, B_n)$.
...
``independently specified by governance'' is satisfied when each
benchmark's acceptance was deliberated separately, which the governance
fork protocol (Definition~\ref{def:gov_fork}) enforces by requiring
per-protocol proposals and per-protocol votes.

\paragraph{Why condition on capability profile.}
Agent $a$'s internal state (training history, memory, strategic
disposition) is a shared cause across all exercises.  The capability
profile $\mathbf{C}_a$ is a sufficient statistic for exercise
outcomes under the completeness and soundness model of
Definition~\ref{def:exercise_protocol}: given $\mathbf{C}_a$, the
agent's performance on each benchmark is characterized by
$(\delta_i, \epsilon_i)$, which are set per-benchmark by governance.
...
the conditional-independence guarantee weakens.  The remark in the
main text addresses this case: even without statistical independence,
the absence of a single-substrate write path across channels is
sufficient for the multi-channel attribution argument.
...
\begin{enumerate}
    ...
    protocol's isolation requirement---the benchmark's soundness
    parameter $\epsilon$ bounds the probability of a false pass.  The
    defense is layered: isolation is the primary barrier, and soundness
    is the backstop for imperfect isolation.  If $\epsilon$ is set
    appropriately (a governance responsibility), residual
    coalition-assisted passing is detectable through re-testing with
    fresh exercise instances.
\end{enumerate}
...
to~2 and is no longer monopolar---the theorem's premise no longer
