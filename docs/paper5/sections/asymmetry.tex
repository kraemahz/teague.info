...
This is Paper~3's substrate partition applied to
verification~\citep{lasser2026horizon}.  The substrate partition
$\{S_1, \ldots, S_m\}$ over a population's agents induces a
corresponding partition on observation channels: channels rooted in
substrate $s'$ are not writable by agents on substrate $s \neq s'$.
Paper~4's multi-channel attribution assumes this independence
structure~\citep{lasser2026scm}; the present paper constructs the
protocol that \emph{enforces} it.
