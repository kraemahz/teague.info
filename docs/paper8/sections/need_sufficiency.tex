...
$G(1) = 1$, $G(0) = 0$, $G$ monotone non-decreasing, and $G$
right-continuous at $s = 0$ (so $\lim_{s \to 0^+} G(s) = 0$).
The four candidate gates of Remark~\ref{rem:gate_candidates} all
satisfy right-continuity; step-function gates are excluded.
...
(Definition~\ref{def:downstream_gate}), and suppose
$s_N(c) \equiv s_N$ for all $c \in \Down(p_N)$ (homogeneous
sufficiency).  Partition the poset as
...
\]
As $s_N \to 0$, $G(s_N) \to 0$ and the downstream contribution
vanishes: $\volL^{\mathrm{eff}} \to W_{\mathrm{out}}$.  When
$W_{\mathrm{down}} \gg W_{\mathrm{out}}$ (most of the agent's
capability-weight lies in the downstream cone), the effective
volume collapses to a small fraction of its unrestricted value.
The multiplicative structure makes need-satisfaction a
precondition for $\volL$ to register the downstream portion of
the agent's capabilities; an agent cannot maximize
$\volL^{\mathrm{eff}}$ without first closing sufficiency gaps on
needs whose downstream cones cover their capability-weight.
\end{proposition}
...
\]
$G$ monotone non-decreasing with $G(0) = 0$ gives
$\lim_{s_N \to 0} G(s_N) = 0$, so the downstream term vanishes.
\end{proof}

\begin{remark}[The non-downstream floor is real]
\label{rem:nondownstream_floor}
The bound has an irreducible $W_{\mathrm{out}}$ term: effective
volume does not collapse \emph{to zero} under unmet need, but
to the weight of capabilities that do not depend on $N$.  The
worked table in Remark~\ref{rem:gate_comparison} exhibits this
explicitly: the three non-downstream capabilities contribute
$3/13 \approx 0.231$ at $s_N = 0$ across all gate choices,
representing $W_{\mathrm{out}} / \volL^{\mathrm{raw}}$ for that
configuration.  The proposition's force comes from the typical
case where needs have broad downstream cones
($W_{\mathrm{down}} \gg W_{\mathrm{out}}$); in that regime the
collapse approaches total in the limit $s_N \to 0$.
\end{remark}
...
$G'(s_N) > 0$, the partial derivative of
$\volL^{\mathrm{eff}}$ with respect to $s_N$ is
\[
...
Adding one unit of weight to any single capability $c'$ outside
$\Down(p_N)$ increases $\volL^{\mathrm{eff}}$ by exactly $1$;
adding it to a capability inside $\Down(p_N)$ increases
$\volL^{\mathrm{eff}}$ by $G(s_N) \leq 1$.  Consequently:

\medskip\noindent
\textbf{Dominance condition.}  The marginal benefit of closing
the sufficiency gap on $N$ exceeds the marginal benefit of unit
weight addition to any single capability whenever
\[
...
Unit weight addition to $c' \notin \Down(p_N)$ contributes
$\Delta \volL^{\mathrm{eff}} = 1 \cdot \prod_{N' \in
\mathcal{N}(c')} G(1) = 1$; to $c' \in \Down(p_N)$ it
contributes $G(s_N) \leq 1$.  The dominance condition
$G'(s_N) \cdot W_{\mathrm{down}}(N) > 1$ therefore suffices.
\end{proof}
...
Proposition~\ref{prop:s1_sufficiency} below, which is
...
\paragraph{The intended direction: capability provenance.}
The failure pattern across attempts (redundancy via automorphism,
detection via dormancy, resolution via bundle structure) is that
every signature over post-declaration data is adversarially
manipulable.  The move that changes the game is examining
capabilities \emph{at declaration}: legitimate capability
acquisition has real cost (training, equipment, time,
apprenticeship, dedicated exercise), and the agent cannot
manufacture provenance without expending real resources unless
the cost structure admits self-dealing, collusion, leverage, or
reuse loopholes.

A \emph{capability provenance} construction---combining
acquisition histories (committed sacrifice events that
established a capability) and exercise histories (time-sacrifice
events through which the capability is maintained)---produces a
cost-weighted individual weight $w_{\mathrm{Prov}}(c) =
\min(w(c), f(\text{cumulative provenance of } c))$ that
collapses to zero for capabilities with neither formal
acquisition nor genuine exercise.  Under an admissible
cost-utilization pair---cost that grows at least as fast as
utilization beyond some scale, with a positive marginal floor---
declaring stuffed capabilities is equilibrium-dominated by
declaring legitimate ones.

Making this claim a theorem requires four axioms the current
sequence does not formally establish: provenance non-reuse
(a sacrifice event cannot justify unbounded capability claims),
non-circular sacrifice (self-dealing and collusive-subsidy
constructions do not count as net provenance), liability-netted
cost (debt-financed sacrifice is netted against the induced
liability), and capability-specific linkage semantics
(cryptographically binding a provenance event to the specific
capability it establishes, extending the commitment protocol
of~\citep[Definition~6]{lasser2026exo}).  It also requires a
provenance extension to cooperative weights (M6), so that
lightly-justified individual nodes do not unlock disproportionate
cooperative credit, and a residual treatment of multi-agent
collusion and external-subsidy cases.

These are not stated as results of this paper.  The stuffing
resolution is thus described here as a \emph{direction}: a named
resolution path that uses machinery already partially present in
the sequence (sacrifice events, commitment protocols, privacy
properties, time-sacrifice valuation), extended by formal
acquisition-event semantics and the four axioms above.  A
dedicated treatment is left to follow-up work.
