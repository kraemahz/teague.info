...
$G(1) = 1$, $G(0) = 0$, $G$ monotone non-decreasing.
...
\Down(p_N)$, so that $s_N$ is a scalar.  For any valid gate
function $G$ differentiable at $s_N$ with $G'(s_N) > 0$, the
partial derivative of $\volL^{\mathrm{eff}}$ with respect to that
scalar $s_N$ (holding all other needs' sufficiency fixed) is:
\[
  \frac{\partial \volL^{\mathrm{eff}}}{\partial s_N}
  \;=\; G'(s_N) \cdot \!\!\sum_{c \in \mathrm{Down}(p_N)}\!\!
  w(c) \cdot \!\!\!\!\prod_{N' \in \mathcal{N}(c),\, N' \neq N}\!\!\!\!
  G\bigl(s_{N'}(c)\bigr).
\]
Adding weight $\delta w$ to a single capability $c'$ increases
$\volL^{\mathrm{eff}}$ by at most $\delta w$.  Therefore, for any
need $N$ with non-trivial downstream cone
$|\mathrm{Down}(p_N)| > 1$ (and all other needs at sufficiency),
the marginal benefit of closing the sufficiency gap on $N$
dominates the marginal benefit of adding weight to any single
capability.  The framework's own optimization pressure prioritizes
need-satisfaction over want-accumulation.
\end{proposition}

\begin{proof}
Under the homogeneous-sufficiency hypothesis, $s_N$ is the single
scalar controlling all gate factors $G(s_N(c))$ for
$c \in \Down(p_N)$.  Capabilities outside the cone satisfy
$N \notin \mathcal{N}(c)$, so their effective weights do not
depend on $s_N$ and vanish under $\partial/\partial s_N$.  For
$c \in \Down(p_N)$, Definition~\ref{def:downstream_gate} gives
\[
  w^{\mathrm{eff}}(c)
  = w(c) \cdot G(s_N) \cdot
  \!\!\!\!\prod_{N' \in \mathcal{N}(c),\, N' \neq N}\!\!\!\!
  G\bigl(s_{N'}(c)\bigr).
\]
Differentiating with respect to $s_N$ and summing over
$c \in \Down(p_N)$:
\[
  \frac{\partial \volL^{\mathrm{eff}}}{\partial s_N}
  = G'(s_N) \cdot \!\!\sum_{c \in \Down(p_N)}\!\! w(c) \cdot
  \!\!\!\!\prod_{N' \in \mathcal{N}(c),\, N' \neq N}\!\!\!\!
  G\bigl(s_{N'}(c)\bigr).
\]
Each gate factor $G(s_{N'}(c)) \in [0, 1]$, so the sum is bounded
above by $\sum_{c \in \Down(p_N)} w(c) = W_{\mathrm{down}}$.

The marginal benefit of adding weight $\delta w$ to a single
capability $c'$ is
$\Delta \volL^{\mathrm{eff}} = \delta w \cdot \prod_{N' \in
\mathcal{N}(c')} G(s_{N'}(c')) \leq \delta w$.  Under the
hypothesis that all other needs are at sufficiency
($s_{N'} = 1$ for $N' \neq N$), every gate product collapses to
$G(s_N)$ for $c \in \Down(p_N)$ and to $1$ otherwise.  The
gradient becomes
$\partial \volL^{\mathrm{eff}}/\partial s_N = G'(s_N) \cdot
W_{\mathrm{down}}$; for any downstream cone with
$W_{\mathrm{down}} > 1$ and $G'(s_N) > 0$, this exceeds the
single-capability marginal benefit of $\delta w$ for any finite
$\delta w \leq W_{\mathrm{down}}$.  Hence closing the sufficiency
gap on $N$ dominates any single-capability weight addition.\qed
\end{proof}
...
\paragraph{Resolution via the need-sufficiency architecture.}
The downstream-safe bundle sufficiency condition
(Definition~\ref{def:downstream_safe}) resolves this tension at
the right structural level.  Measurements that improve the
bundle's downstream-safety are genuine: they close sufficiency
gaps on need dimensions previously unmet, and their contribution
is registered through the capped below-sufficiency benchmark
(Definition~\ref{def:polarity_benchmark}).  Measurements that
re-measure an already-sufficient bundle from a different vantage
point are stuffed: they improve no dimension's sufficiency
status, create no new cooperative capability, and contribute no
quality improvement to the downstream cone.  Proposition~3 below
makes this formal.
...
constructions---e.g., ``travel from $A$ to a gas
station,'' ``travel from $B$ to a gas station,'' instantiating the
same underlying fuel-access optionality from different
origins---provide no downstream-quality
...
not of individual capability constructions.  A stuffed capability
instantiates a dimension already at or above threshold for the
need it targets.  By the capping discipline
(Definition~\ref{def:polarity_benchmark}), its below-sufficiency
benchmark is $\min(\mathrm{actual}/\mathrm{threshold}, 1) = 1$.
Its above-sufficiency benchmark provides credit only if
$\mathrm{actual}_k$ genuinely exceeds the threshold, which
re-instantiation from a different origin does not achieve.
Downstream non-degradation
(Definition~\ref{def:downstream_safe}(b)) is unaffected because
the satisfaction pathway is unchanged.\qed
\end{proof}
...
Theorem~\ref{thm:aggregate_bound}.\qed
\end{proof}
