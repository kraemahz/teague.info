...
(Definition~\ref{def:downstream_gate}).  For an agent whose
cooperative capabilities all lie in $\mathrm{Down}(p_N)$ for
some need $N$, if the agent is at sufficiency level $s_N < 1$:
\[
  \volL^{\mathrm{eff}}(G) \;\leq\; G(s_N) \cdot \volL^{\mathrm{raw}}.
\]
As $s_N \to 0$, $G(s_N) \to 0$ and the agent's effective
$\volL$ collapses toward zero regardless of capability breadth.
The multiplicative structure makes need-satisfaction a
precondition for $\volL$ to register the agent's capabilities at
all; an agent cannot maximize $\volL^{\mathrm{eff}}$ without first
closing sufficiency gaps on needs whose downstream cones cover
their cooperative capabilities.
\end{proposition}

\begin{proof}[Proof sketch]
Every cooperative capability $c$ has $N \in \mathcal{N}(c)$ by
assumption, so its effective weight is bounded above by $w(c)
\cdot G(s_N)$.  Summing:
\[
  \volL^{\mathrm{eff}} = \sum_c w^{\mathrm{eff}}(c)
  \leq \sum_c w(c) \cdot G(s_N) = G(s_N) \cdot \volL^{\mathrm{raw}}.
\]
$G$ monotone non-decreasing with $G(0) = 0$ gives
$\lim_{s_N \to 0} G(s_N) = 0$.\qed
\end{proof}

\begin{proposition}[Need-Dominance in the Gradient]
\label{prop:need_dominance}
For any valid gate function $G$ with $G$ differentiable at
$s_N$ and $G'(s_N) > 0$:
\[
...
$\volL^{\mathrm{raw}} = 13$.  At sufficiency level $s$,
...
The canonical form is redundant measurement of a single
underlying need from multiple vantage points.  ``Distance from
home to gas station'' is a real, benchmarkable capability: it
satisfies the axioms M1--M6, has a well-defined metric
($s_{\max} \geq 1$), and represents genuine structural knowledge
about the world.  Adding ``distance from work to gas station,''
``distance from grocery store to gas station,'' and so on produces
a sequence of further real, benchmarkable capabilities, each
passing the same axiomatic checks.  Collectively they inflate
$\volL$ without broadening what the agent can actually do: every
addition measures the same underlying optionality
(fuel access) from a different geometric position.
...
\emph{B-to-C gap} in the aggregate sense that Paper~8 otherwise
...
of~\citep[Definition~5]{lasser2026horizon}, which holds that
...
making a structural discovery in Paper~3's sense.  The formal
