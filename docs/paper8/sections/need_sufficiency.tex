...
\begin{proof}[Proof sketch]
Direct differentiation.  Only capabilities in
$\mathrm{Down}(p_N)$ depend on $s_N$; the product rule isolates
$G'(s_N)$ times the weighted sum of other-gate products.  The
single-capability comparison follows from $w^{\mathrm{eff}}(c')
\leq w(c')$ for all $c'$.\qed
\end{proof}
