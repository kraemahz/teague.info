...
\medskip\noindent
\textbf{Cooperative-closure hypothesis.}  We additionally require
the partition $\{R, C, D, S\}$ to be \emph{cooperative-closed}:
for every cooperative capability $z \in P$, all participants of
$z$ lie in the same cell.  Set-disjointness alone does not
suffice for $\volL$-additivity across cells because $\volL$
counts cooperative contributions only when all participants are
in the same subset (Remark~\ref{rem:poset_indep}); a cooperative
whose participants straddle two cells contributes to $\volL(P)$
but to neither $\volL$-cell individually.  Cooperative-closure
ensures no such straddling and licenses additive decomposition
by axiom~M4 (Section~\ref{sec:axiom_inheritance}).  See
Remark~\ref{rem:cell_cooperative_closure} for how to achieve
cooperative-closure in practice.
...
\end{align*}
Under cooperative-closure, the identity $1 - \betaL = \sum \delta$
follows from $\volL(G) = \volL(R) + \volL(C) + \volL(D) +
\volL(S)$ (cooperative-closure + set-disjoint union is $P$, via
M4), combined with $\volRlower(P) = \volRlower(R) +
\volRlower(C) + 0 + 0$ (dormant and residual cells contribute
zero $\volRlower$ by definition).
\end{definition}

\begin{remark}[Achieving cooperative-closure of the cells]
\label{rem:cell_cooperative_closure}
Three operational strategies admit the cooperative-closure
hypothesis without changing the framework's substantive content:
\begin{enumerate}
\item[(a)] \emph{Priority-extension to cooperatives.}  For each
  cooperative $z \in P$ not already classified by priority rule,
  place $z$ in the cell of its highest-priority participant
  (restricted $>$ covered $>$ dormant $>$ residual).  When
  $z$'s participants all share a cell, $z$ joins them; when
  they straddle, $z$ follows the dominant priority.  The cells
  become cooperative-closed by construction, and the underlying
  individual-capability classification is unchanged.
\item[(b)] \emph{Cooperative-closed sub-partition.}  Refine the
  partition so each cell is partitioned into its cooperative
  closures: the set of connected components under the
  cooperative-composition relation.  The gap decomposition then
...
\item[(c)] \emph{Boundary residual.}  Add a fifth bucket for
  boundary cooperatives---cooperatives whose participants span
  cells---and attribute its $\volL$-mass to a residual term.
  ...
  unassigned cooperative mass.
\end{enumerate}
The paper's stated results assume strategy~(a) by default: the
priority-extension rule is the natural operational choice, and
the practical prevalence of boundary cooperatives is small when
the capability partition's priority classes are themselves
well-structured (e.g., restrictions are cooperative-closed
because restricting a cooperative implies restricting its
participants).
\end{remark}
...
the cooperative-composition structure $\mathcal{M}$
(enumeration of cooperative nodes and their participants),
...
(Theorem~\ref{thm:aggregate_bound}), the full Part~B
admissibility set is required: S0, S1, S2, S4(a), S3, S5, the
genuine-exchange conditions (i)--(iv) of
Definition~\ref{def:sacrifice_event}, \emph{and} within-bundle
poset-independence per Definition~\ref{def:bundle_decomp}; and
...
is the number of cooperative nodes in the poset.  The $O(|P|)$
term is per-individual-capability classification; the $O(E)$
term is one-pass aggregation over bundle-incidence records; the
$O(M)$ term is the closure-policy pass over cooperative nodes.
...
Definition~\ref{def:bundle_decomp}---update $c$'s class from
...
For each cooperative node $z \in \mathcal{M}$ with participants
$\Pi(z) \subseteq P$: under default strategy~(a), place $z$ in
the cell of its highest-priority participant (restricted $>$
covered $>$ dormant $>$ residual).  Look up each participant's
cell (at most $|\Pi(z)| = O(1)$ for bounded-arity cooperatives,
$O(\max_z |\Pi(z)|)$ in general); assign $z$ to the maximal
priority.  The cells become cooperative-closed
(Definition~\ref{def:gap_decomp}) by construction.  Under
strategy~(b) or~(c), the pass is replaced by the corresponding
sub-partition or boundary-bucket construction, with the same
$O(M)$ complexity modulo constants.
