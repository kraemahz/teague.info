...
(Theorem~\ref{thm:aggregate_bound}) actually bounds: each
observed event at $t_n \in W$ witnesses exercise of $Y_n$ at
that time, so its contribution is aggregated into $\volR^{[W]}$,
not into $\volR_t$ at any single later evaluation time.
\end{definition}
...
(valuation-bounded-by-exercise), $\Ui[_{n}](Y_n) \leq
\volR_{t_n}(Y_n)$, evaluated at the exercise time $t_n$; since
$t_n \in W$, $P^{\mathrm{ex}}_{t_n} \subseteq P^{\mathrm{ex}}_W$
and hence $\volR_{t_n}(Y_n) \leq \volR^{[W]}(Y_n)$ by monotonicity
of $\volL$.  By S2 (benchmark grounding), $\Ui[_{n}](X_n) \geq
\Delta\volL(X_n)$.  Chaining:
\[
  \volR^{[W]}(Y_n) \;\geq\; \volR_{t_n}(Y_n)
  \;\geq\; \Ui[_{n}](Y_n) \;\geq\; \Ui[_{n}](X_n)
  \;\geq\; \Delta\volL(X_n),
\]
which is the per-event bound.  The window-integrated form is
essential: $Y_n$ was witnessed as exercised at $t_n$, which may
differ from any downstream evaluation time; $\volR^{[W]}$
aggregates such witnesses across $W$, whereas $\volR_t$ at a
later $t$ could have dropped below $\Delta\volL(X_n)$ if
$Y_n$ is no longer exercised.
