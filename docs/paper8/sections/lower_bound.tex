...
$\volL$~\citep[Proposition~1]{lasser2026poset}.  For each time
$t$, the \emph{exercised sub-poset} $P^{\mathrm{ex}}_t
\subseteq P$ is the set of capabilities currently being
exercised at time $t$---a \emph{latent structural fact} about
the system, independent of any observation mechanism:
\[
  P^{\mathrm{ex}}_t \;=\; \{d \in P : d \text{ is currently
  exercised at time } t\},
\]
where ``currently exercised'' is formalized via the counterfactual
exercise condition of Appendix~\ref{app:exercise_indicator}
(Definition~\ref{def:exercise_indicator}): $d$ is exercised iff
removing $d$ at time $t$ would make some realized cooperative
output unrealizable.  The \emph{realized capability volume} is
the poset measure restricted to this sub-poset:
\[
  \volR(S) \;=\; \volL\bigl(S \cap P^{\mathrm{ex}}_t\bigr)
  \qquad \text{for any } S \subseteq P.
\]
$\volR$ inherits the measure structure of $\volL$ restricted to
$P^{\mathrm{ex}}_t$, and the axiom-analogue analysis of
Section~\ref{sec:axiom_inheritance} applies.
\end{definition}

\begin{remark}[$\volR$ is latent; $\volRlower$ is the bound]
\label{rem:volR_diagnostic}
$\volR$ is defined structurally as a ground-truth quantity
about the system; computing it \emph{directly} requires
evaluating the counterfactual exercise condition for every
capability, which the Introduction rejects as intractable and
privacy-incompatible at scale.  The revealed-sacrifice
observation channel does not observe $P^{\mathrm{ex}}_t$
directly; it produces a lower bound $\volRlower$ on
$\volR$ restricted to an observed sub-space $U \subseteq P$.
Inside the observation perimeter $O$ of
...
(Appendix~\ref{app:exercise_indicator}).  $\volR$ itself is the
