...
\emph{Part~B.}
Fix an event $n$ and its bundle $Y_n$.  By S3 (bundle coherence),
the hedonic decomposition of $Y_n$ is non-degenerate; by S4(b)
applied within the bundle, $\volR$ is additive over the
poset-independent capabilities it contains:
\[
  \volR(Y_n) \;=\; \sum_{c \in Y_n} \volR(c).
\]
By S5, the event-local coefficients are lower bounds on the true
share: $\alpha_{n,c} \leq \volR(c)/\volR(Y_n)$.  Rearranging and
applying the Part~A per-event bound:
\[
  \volR(c) \;\geq\; \alpha_{n,c} \cdot \volR(Y_n)
  \;\geq\; \alpha_{n,c} \cdot \Delta\volL(X_n),
\]
for every $c \in Y_n$.  If $c$ appears in multiple observed
...
\[
  ...
  \;\geq\; \sum_{c \in U} \volRlower(c). \qed
\]
\end{proof}
...
$\volRlower^{(N')} \geq \volRlower^{(N)}$.\qed
\end{proof}
