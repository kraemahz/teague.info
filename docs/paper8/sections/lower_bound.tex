...
a subset $U$ of unbenchmarked capability-space, under the
...
\item[S1 (Free choice).] Each sacrifice event is a voluntary
  ...
  dimension.  When retaining $X_n$ would cause the agent to fall
  below a sufficiency threshold, S1 fails for that event
...
\item[S5 (Decomposition validity).]  The event-local
  decomposition coefficients are \emph{lower bounds} on the true
  $\volR$ share:
  $\alpha_{n,c} \leq \volR(c) / \volR(Y_n)$ for all $n$ and
  $c \in Y_n$.
\end{description}

The theorem has two levels:
