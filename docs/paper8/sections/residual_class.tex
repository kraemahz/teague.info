...
(Definition~\ref{def:exercise_indicator}), the OI floor
contributes exclusively to $\volRexact(O)$ in the combined
diagnostic (Appendix~\ref{app:exercise_indicator}):
\[
...
would double-count.  When boundary-closure fails, $\volRcomb$
remains a conservative lower bound on $\volR(P)$ but undercounts
cooperatives spanning the perimeter
(Remark~\ref{rem:boundary_cooperative_undercount}); the OI
floor's contribution is unaffected because the OI capability
$d_k$ is, by construction, an internal capability of agent $k$'s
substrate---not a cooperative spanning the perimeter boundary.
\end{remark}
