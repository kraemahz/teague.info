...
steps, each grounded in existing framework structure rather than
a new posit.
...
\item \textbf{Reduction to bundle-level comparison (via M4 + S0
  calibration).}
  The poset measure $\volL$ is additive over poset-disjoint
  components by axiom M4~\citep[Proposition~1]{lasser2026poset},
  with complementarities handled by M6 as explicit cooperative
  nodes.  S0 and S2 calibrate $\Ui$ to $\volR$ and $\volL$ on
  the acquisition and sacrifice sides respectively; under this
  calibration, $\Ui$ inherits the poset measure's additivity
  structure over the same partition.  Consequently the exchange
  comparison reduces to a bundle comparison:
  $\Ui\bigl((H \setminus X) \cup Y\bigr) \geq \Ui(H)
  \iff \Ui(Y) \geq \Ui(X)$.
  (The additivity here is not a new assumption: it is the
  structural consequence of the calibration inequalities together
  with M4.  Complementarities that would break pure additivity
  in general consumer theory are absorbed into the poset as
  cooperative nodes via M6, which are themselves capabilities
...
\noindent
Step~2's reduction to bundle-level comparison is where this
derivation differs from the textbook revealed-preference
inference, which typically assumes preference additivity as a
starting axiom.  Here, additivity is derived: from M4 applied
to the poset measure, through the calibration inequalities of
S0 and S2, to $\Ui$'s behaviour on capability partitions.
