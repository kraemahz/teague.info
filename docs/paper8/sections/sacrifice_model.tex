...
capability set $A$.  The post-trade holding set decomposes as a
disjoint union by (ii): $(H \setminus X) \cup Y = (H \setminus
X) \sqcup Y$.  Additivity gives
\[
  \Ui\bigl((H \setminus X) \cup Y\bigr)
  = \sum_{c \in H \setminus X} u_i(c) + \sum_{c \in Y} u_i(c).
\]
The pre-trade holding set decomposes as $H = (H \setminus X)
\sqcup X$ (since $X \subseteq H$), so
$\Ui(H) = \sum_{c \in H \setminus X} u_i(c) + \sum_{c \in X}
u_i(c)$.
Subtracting, the $H \setminus X$ terms cancel, leaving
$\Ui((H \setminus X) \cup Y) - \Ui(H) = \Ui(Y) - \Ui(X)$.
Condition (iii) ensures that the cooperative capabilities
implicitly included in each term (via S4(a)) are those
formable within $Y$ alone or within $H \setminus X$ alone;
no cross-set cooperative capability appears in
$(H \setminus X) \cup Y$ without already appearing in one of
the pre-existing summands.  Condition (i) makes $\Ui(Y)$ and
$\Ui(X)$ refer to distinct capability sets; violating (i) would
cause $X \cap Y$ terms to cancel trivially on both sides,
...
containment $Y_n \subseteq C_{\catY[_n]}$
...
Part~A aggregate bound is computable from committed events
together with the category-partition structure.  The committed
form reveals only the sacrifice magnitude and bundle category;
agent identity, specific $X$ and $Y$, and counterparty remain
hidden by the hiding property of
