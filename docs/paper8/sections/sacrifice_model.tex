...
\item \textbf{Reduction to bundle-level comparison via valuation
  additivity (S4(a)).}
  Under S4(a)---the assumption that $\Ui$ is additive across
  any capability set (stated below)---the exchange comparison
  ...
  (\citet{samuelson1938note,mascolell1995microeconomic}), and
  the paper uses a special case implicitly already: the
  time-sacrifice channel (Definition~\ref{def:time_sacrifice})
  values time at wage rate $r_i$, which is quasilinearity in
  wage-scaled time---a form of valuation additivity.
