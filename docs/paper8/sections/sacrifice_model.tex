...
of $Y$ in context rather than $\volR(Y)$ in isolation---the bound
remains conservatively valid for the contextual quantity but
the theorem's isolated-$\volR(Y)$ interpretation requires all
four conditions.
...
\item[S0 (Valuation-bounded-by-Exercise).] For each agent~$i_n$,
  the agent's valuation of the unbenchmarked bundle $Y_n$ does
  not exceed $Y_n$'s realized $\volR$ contribution:
  $\Ui[_{n}](Y_n) \leq \volR(Y_n)$.
  This is the \emph{substantive bridge} between valuation (what
  the agent pays for) and realized exercise (what the framework
  cares about).  It is not a calibration-in-the-narrow-sense
  (unbiased estimation) but a one-sided conservatism claim:
  agents do not systematically pay more than what the capability
  will actually deliver in exercise.
