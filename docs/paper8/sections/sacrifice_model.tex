...
\item $Y = \{Y_1, \ldots, Y_k\}$ is an unbenchmarked capability
  ...
  holdings); and
  (iii)~\emph{cooperative closure}: no new cooperative capability
  is activated by combining $Y$ with $H_i \setminus X$ that was
  not already activatable within $Y$ alone or within
  $H_i \setminus X$ alone.  Formally, every cooperative capability
  $c_{\mathrm{coop}}$ in the poset whose participants are all in
  $(H_i \setminus X) \cup Y$ has its participants either all in
  $Y$ or all in $H_i \setminus X$;
...
novel.  Condition (iii) ensures the exchange-to-bundle
cancellation in the revealed-preference derivation is exact: if
new cross-bundle cooperative capabilities activated, the agent
would reveal willingness-to-pay for $Y$-plus-cooperatives, not
for $Y$ alone.  Trades violating (i) or (ii) are degenerate;
trades violating (iii) carry revealed-preference information
about the marginal contribution of $Y$ in context, which may
exceed $\volR(Y)$ as an isolated bundle---the bound still holds
conservatively for the contextual quantity but the theorem's
isolated-$\volR(Y)$ interpretation requires (iii).
...
that reduces the revealed-preference inequality from a state
comparison to a bundle comparison.
\end{definition}
...
\item \textbf{Reduction to bundle-level comparison via valuation
  ...
  (disjointness (i)--(ii) plus cooperative closure (iii)), the
  exchange comparison reduces by cancellation to a bundle
  comparison:
  \[
  ...
  \iff \Ui(Y) \geq \Ui(X)$.  All three conditions contribute:
  \begin{itemize}
  \item (i) rules out the $X \cap Y$ overlap that would cancel
    trivially and yield no information.
  \item (ii) rules out the $Y \cap (H \setminus X)$ overlap
    that would produce no new utility because the agent already
    held those capabilities.
  \item (iii) rules out cooperative-cross-term activations:
    without it, $\Ui((H \setminus X) \cup Y)$ would include
    utility from new cooperative capabilities formed by combining
    $Y$-elements with $H \setminus X$-elements, breaking the
    cancellation (the agent's willingness-to-pay would reflect
    the marginal contribution of $Y$ in context rather than
    $\Ui(Y)$ in isolation).
  \end{itemize}
  S4(a) is a global additive-separability assumption on $\Ui$.
  It is stronger than the rationality-only premises of
  Samuelson's original revealed-preference
  theorem~\citep{samuelson1938note}, which establishes
  preferences over states rather than additivity over bundles.
  In applied microeconomics~\citep{mascolell1995microeconomic},
  bundle-level inference of the kind the sacrifice channel
  performs typically invokes either additive separability or
  quasilinearity in a numeraire.  The paper uses a special case
  implicitly via the time-sacrifice channel
  (Definition~\ref{def:time_sacrifice}), which values time at
  wage rate $r_i$---quasilinearity in wage-scaled time, a form
  of valuation additivity.  S4(a) is the explicit assumption
  that makes the exchange-to-bundle cancellation go through; it
  is not a consequence of S0--S3.
...
\item[\textbf{(P5)}] \textbf{Third-party observability
  (institutional).}
  Money-sacrifice events and publicly observable time-sacrifice
  events are observable from public-facing ledger data (market
  exchanges, payment rails, public commitments) with no agent
  participation required, \emph{assuming} such public-ledger
  infrastructure exists.  Where it does not, P5 fails
  operationally even though the channel's construction is
  unchanged.  This contrasts with controlled
  ...
  disclosure regardless of ledger infrastructure.
\end{enumerate}
\end{proposition}
