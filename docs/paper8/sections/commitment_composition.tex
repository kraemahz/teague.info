...
\begin{proof}
The prover holds the full set of $N$ committed events
$\{(\Gamma_n, \pi_n, t_n)\}$ together with the openings
$(\Delta\volL(X_n), \catY[_n], r_n)$ witnessed by the
proof-of-trade~$\pi_n$.  The rollup proof $\Pi$ establishes (i)
and (ii) simultaneously as follows.
...
demonstrates
$\sum_{n=1}^N \Delta\volL(X_n) \geq B$ without revealing the
individual terms or $N$.  The summation commitment is the product
$\prod_n \Gamma_n$, which binds to the aggregate by the
homomorphic property of $\Commit$.

\emph{Component (ii)---pairwise-distinct category labels.}
The prover commits to the category set
$\mathcal{C} = \{C_1, \ldots, C_m\}$ as a public Merkle root.
For each event, the ZK proof shows (a) that $\catY[_n]$ opens to
a leaf of $\mathcal{C}$ and (b) that the multiset
$\{\catY[_n]\}_{n=1}^N$ contains no duplicate leaves, via a
set-membership argument over a sorted commitment vector.  The
duplicate-freeness argument is a standard ZK disjointness proof:
commit to sorted $(\catY[_1], \ldots, \catY[_N])$, prove
strict-monotone ordering element-by-element, and prove the
permutation relation to the unsorted commitments using a
shuffle argument~\citep[Definition~7]{lasser2026exo}.

\emph{Invoking Theorem~\ref{thm:aggregate_bound}.}
By Definition~\ref{def:category_partition}(i), pairwise-distinct
labels imply pairwise poset-independent bundles.  Combined with
(i), the verifier concludes $\volR^U \geq B$ without learning
any individual $\Delta\volL(X_n)$, the labels, or~$N$.\qed
\end{proof}
