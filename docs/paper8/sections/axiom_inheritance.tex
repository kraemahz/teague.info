...
\item[(M6) Superadditivity under independence.]
  ...
  \item[(b)] \textbf{$\volRlower$ fails M6 (observational).}
    ...
    Example~\ref{ex:m6_fail_volRlower} witnesses this.
  \end{enumerate}
\end{description}
\end{theorem}
...
mapping is not monotone under capability-set expansion
($P \supset P'$ but $\volR(P) = \volR(P')$).  Equivalently, the
...
relies on the capability-removal non-invariance of M6(a), not
on this lower-bound subadditivity.
\end{example}
