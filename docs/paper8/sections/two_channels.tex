...
Strict inequality is established by the non-fungibility
residuals: agent-specific-time capabilities
(Proposition~\ref{prop:agent_specific_time}) lie in the coverage
gap of the money channel alone, and capital-concentrated
capabilities (Proposition~\ref{prop:capital_concentrated}) lie in
the coverage gap of the time channel alone, giving explicit
witnesses to strict improvement in each direction.  For
capabilities outside the non-fungibility residuals, whether the
joint aggregate improves strictly over a single channel depends
on the data coverage of the trade window---substitutability via
cooperative-market exchange (\S\ref{sec:channel_substitutability})
means a single channel could in principle match the joint
aggregate if the agent population uses that channel
exclusively, but realistic populations use both modalities
heterogeneously and the joint aggregate strictly improves in
most trade windows.
\end{corollary}
...
Strict improvement on the residuals:
