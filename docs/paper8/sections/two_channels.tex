...
channels (no capability appears in bundles from both channels
within $W$).
...
ensures Part~A's aggregation is well-defined on $E$; the
aggregate is monotone in observed events
(Proposition~\ref{prop:monotone}).  Strict improvement on the
...
\begin{remark}[Cross-channel capability overlap]
\label{rem:cross_channel_overlap}
The closure hypothesis rules out a capability $c$ appearing in
both a money-channel bundle and a time-channel bundle during the
same window $W$ (which would violate Part~A's pairwise
poset-independence).  When cross-channel overlap occurs in
practice, the operational responses of
Remark~\ref{rem:label_collision} apply: partition events into
maximal non-overlapping sub-batches per channel and aggregate
separately, or fall back to Part~B's per-capability max-attribution
(which natively handles overlapping events via the max across
events covering each capability).
\end{remark}
