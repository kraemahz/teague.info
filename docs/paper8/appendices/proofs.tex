...
\paragraph{Proposition~\ref{prop:anti_stuffing} (Anti-Stuffing).}
Under bundle sufficiency, redundant measurements contribute to
already-met dimensions.  Below-sufficiency benchmark caps at $1$
for met dimensions
(Definition~\ref{def:polarity_benchmark}).
Above-sufficiency benchmark credits only genuine quality
improvement, which re-measurement does not provide.  Downstream
non-degradation (Definition~\ref{def:downstream_safe}(b)) is
unchanged by re-measurement.
...
\begin{enumerate}
\setcounter{enumi}{2}
\item S4(b) decomposes $\volR(Y_n) = \sum_c \volR(c)$.  S5 gives
  $\alpha_{n,c} \leq \volR(c)/\volR(Y_n)$, yielding
...
Same transfer argument as the prior formulation.  Active agents
generate the OI floor
