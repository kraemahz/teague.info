...
\item Calibration chain (S0 + S2) with window-integration step:
  $\volR^{[W]}(Y_n) \geq \volR_{t_n}(Y_n) \geq \Ui[_{n}](Y_n)
  \geq \Ui[_{n}](X_n) \geq \Delta\volL(X_n)$.  The
  window-integrated form is essential: $Y_n$ is witnessed as
  exercised at $t_n \in W$, so its contribution enters
  $\volR^{[W]}$ but need not persist in $\volR_t$ at a later
  evaluation time (Definition~\ref{def:volR}).
